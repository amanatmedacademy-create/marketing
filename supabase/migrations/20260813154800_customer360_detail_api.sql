...
begin
  if not exists (select 1 from public.crm_contacts where id=p_contact_id and company_id=p_company_id and deleted_at is null) then return null; end if;

  select jsonb_build_object(
    'contact', jsonb_build_object(
      'id',c.id,'fullName',coalesce(nullif(btrim(concat_ws(' ',c.first_name,c.last_name)),''),nullif(c.phone,''),nullif(c.email,''),'Без имени'),
      'firstName',c.first_name,'lastName',c.last_name,'phone',c.phone,'email',c.email,'source',c.source,'createdAt',c.created_at,'updatedAt',c.updated_at
    ),
    'leads', coalesce((select jsonb_agg(jsonb_build_object(
      'id',l.id,'name',l.name,'phone',l.phone,'email',l.email,'source',l.source,'platform',l.platform,'stage',l.stage,
      'createdAt',l.created_at,'updatedAt',l.updated_at,'dealId',l.crm_deal_id
    ...
    'deals', coalesce((select jsonb_agg(jsonb_build_object(
      'id',d.id,'title',d.title,'phone',d.phone,'email',d.email,'source',d.source,'status',d.status,'amount',d.amount,'currency',d.currency,
      'paid',d.paid,'priority',d.priority,'nextAction',d.next_action,'nextActionAt',d.next_action_at,'stageId',d.stage_id,'stageName',s.name,
      'pipelineId',d.pipeline_id,'pipelineName',p.name,'marketingLeadId',d.marketing_lead_id,'createdAt',d.created_at,'updatedAt',d.updated_at
    ...
    'calls', coalesce((select jsonb_agg(jsonb_build_object(
      'id',x.id,'leadId',x.lead_id,'conversationId',x.conversation_id,'phone',x.client_phone,'status',x.call_status,'channel',x.channel,
      'direction',x.call_direction,'startedAt',x.started_at,'durationSeconds',x.duration_seconds,'result',x.call_result,'summary',x.summary,
      'nextAction',x.next_action,'recordingUrl',x.recording_url
    ...
    'conversations', coalesce((select jsonb_agg(jsonb_build_object(
      'id',x.id,'leadId',x.lead_id,'title',x.title,'phone',x.phone,'channel',x.channel,'status',x.status,'unreadCount',x.unread_count,
      'lastMessageAt',x.last_message_at,'assignedUserId',x.assigned_user_id
    ) order by x.last_message_at desc nulls last) from public.marketing_conversations x where x.company_id=p_company_id and x.contact_id=c.id and x.archived_at is null), '[]'::jsonb),
    'messages', coalesce((select jsonb_agg(jsonb_build_object(
      'id',m.id,'conversationId',m.conversation_id,'body',m.body,'direction',m.direction,'senderName',m.sender_name,'status',m.status,
      'sentAt',m.sent_at,'attachmentName',m.attachment_name
    ...
    'appointments', coalesce((select jsonb_agg(jsonb_build_object(
      'id',a.id,'leadId',a.lead_id,'conversationId',a.conversation_id,'patientId',a.patient_id,'patientName',a.patient_name,'phone',a.phone,
      'status',a.status,'source',a.source,'startsAt',a.starts_at,'endsAt',a.ends_at,'doctorId',a.doctor_id,'branchId',a.branch_id
    ...
    'journey', coalesce((select jsonb_agg(jsonb_build_object(
      'id',e.id,'leadId',e.lead_id,'type',e.event_type,'occurredAt',e.occurred_at,'channel',e.channel,'source',e.source,'value',e.value,
      'currency',e.currency,'campaignId',e.campaign_id,'adsetId',e.adset_id,'adId',e.ad_id
    ...
    'tasks', coalesce((select jsonb_agg(jsonb_build_object(
      'id',t.id,'title',t.title,'description',t.description,'status',t.status,'priority',t.priority,'dueAt',t.due_at,'completedAt',t.completed_at,
      'assigneeId',t.assignee_id,'linkType',t.link_type,'linkId',t.link_id,'linkLabel',t.link_label
    ) order by t.due_at asc nulls last, t.created_at desc) from public.crm_tasks t where t.company_id=p_company_id and (
  ...
  from public.crm_contacts c where c.id=p_contact_id and c.company_id=p_company_id and c.deleted_at is null;
  return result;
end;
$$;
