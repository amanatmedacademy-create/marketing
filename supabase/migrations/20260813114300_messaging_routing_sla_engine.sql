...
declare
  ...
  v_config jsonb;
begin
  if new.conversation_id is null or new.company_id is null then return new; end if;

  select r.config into v_config
  from public.crm_task_automation_rules r
  where r.company_id = new.company_id and r.key = 'messaging_sla'
  limit 1;
  ...
  if upper(coalesce(new.direction, '')) = 'INBOUND' then
    select c.assigned_user_id into v_assignee
    ...
    if v_assignee is null and v_auto_assign then
      ...
      from (
        select m.user_id,
          (select count(*) from public.marketing_conversations c2
            where c2.company_id = new.company_id
              and c2.assigned_user_id = m.user_id
              and c2.archived_at is null
              and coalesce(c2.status, 'OPEN') <> 'CLOSED') as open_load,
          u.last_seen_at
        from public.crm_company_members m
        join public.marketing_users u on u.id = m.user_id and u.status = 'active'
        left join public.crm_access_user_assignments a on a.company_id = m.company_id and a.user_id = m.user_id
        left join public.crm_access_positions p on p.company_id = a.company_id and p.id = a.position_id
        where m.company_id = new.company_id
          and m.status = 'active'
          and m.role in ('owner', 'administrator', 'manager')
        order by case when p.system_key = 'call_center' then 0 else 1 end,
          open_load, u.last_seen_at desc nulls last, m.user_id
        limit 1
      ) candidate;
    end if;
    ...
    set last_inbound_at = new.sent_at,
        ...
        routing_assigned_at = case when c.assigned_user_id is null and v_assignee is not null then now() else c.routing_assigned_at end,
        routing_strategy = case when c.assigned_user_id is null and v_assignee is not null then 'least_loaded' else c.routing_strategy end,
...
create trigger marketing_messages_sync_conversation_state after insert on public.marketing_messages
...
set last_inbound_at = s.last_inbound_at,
    ...
    awaiting_reply_since = case
      when s.last_inbound_at is not null
       ...
       and coalesce(c.status, 'OPEN') <> 'CLOSED' then s.last_inbound_at else null end,
    sla_due_at = case
      when s.last_inbound_at is not null
       and s.last_inbound_at > coalesce(s.last_outbound_at, '-infinity'::timestamptz)
       and coalesce(c.status, 'OPEN') <> 'CLOSED' then s.last_inbound_at + interval '5 minutes' else null end,
    sla_escalation_level = 0
from message_state s
where s.conversation_id = c.id;

insert into public.crm_task_automation_rules(company_id, key, name, description, enabled, config)
select distinct c.id, 'messaging_sla', 'Messaging SLA и маршрутизация',
  'Автоназначение входящих диалогов, контроль первого ответа и двухуровневая SLA-эскалация с задачами.',
  true, jsonb_build_object('firstResponseMinutes', 5, 'escalationMinutes', 15, 'autoAssign', true)
from public.crm_companies c
on conflict (company_id, key) do nothing;

update public.crm_task_automation_rules
set enabled = false, updated_at = now()
where key = 'whatsapp_unanswered' and enabled = true;
...
begin
  insert into public.crm_task_automation_rules(company_id, key, name, description, enabled, config)
  select distinct c.company_id, 'messaging_sla', 'Messaging SLA и маршрутизация',
    'Автоназначение входящих диалогов, контроль первого ответа и двухуровневая SLA-эскалация с задачами.',
    true, jsonb_build_object('firstResponseMinutes', 5, 'escalationMinutes', 15, 'autoAssign', true)
  from public.marketing_conversations c where c.company_id is not null
  on conflict (company_id, key) do nothing;

  for v_conv in
    select c.* from public.marketing_conversations c
    where c.archived_at is null
      and c.awaiting_reply_since is not null
      and coalesce(c.status, 'OPEN') <> 'CLOSED'
    order by c.awaiting_reply_since asc limit 1000
  loop
    v_scanned := v_scanned + 1;
    select r.enabled, r.config into v_enabled, v_config
    from public.crm_task_automation_rules r
    where r.company_id = v_conv.company_id and r.key = 'messaging_sla' limit 1;
    if coalesce(v_enabled, true) is false then continue; end if;
    ...
    if v_assignee is null and coalesce((v_config ->> 'autoAssign')::boolean, true) then
      ...
      from (
        select m.user_id,
          (select count(*) from public.marketing_conversations c2
            where c2.company_id = v_conv.company_id and c2.assigned_user_id = m.user_id
              and c2.archived_at is null and coalesce(c2.status, 'OPEN') <> 'CLOSED') as open_load,
          u.last_seen_at
        from public.crm_company_members m
        join public.marketing_users u on u.id = m.user_id and u.status = 'active'
        left join public.crm_access_user_assignments a on a.company_id = m.company_id and a.user_id = m.user_id
        left join public.crm_access_positions p on p.company_id = a.company_id and p.id = a.position_id
        where m.company_id = v_conv.company_id and m.status = 'active'
          and m.role in ('owner', 'administrator', 'manager')
        order by case when p.system_key = 'call_center' then 0 else 1 end,
          open_load, u.last_seen_at desc nulls last, m.user_id limit 1
      ) candidate;
      if v_assignee is not null then
        update public.marketing_conversations
        set assigned_user_id = v_assignee, routing_assigned_at = now(), routing_strategy = 'least_loaded', updated_at = now()
        where id = v_conv.id and assigned_user_id is null;
      end if;
    end if;
    ...
    if now() >= v_conv.awaiting_reply_since + make_interval(mins => v_first)
      ...
      insert into public.crm_tasks(company_id,title,description,status,stage_key,workflow_key,priority,due_at,sla_minutes,sla_due_at,source,assignment_mode,link_type,link_id,link_label,automation_key,automation_event_key)
      values (v_conv.company_id,'SLA: ответить клиенту','Клиент ожидает ответ в Messaging дольше установленного SLA.','todo','new','call_center','urgent',now(),5,now()+interval '5 minutes','work_tasks',case when v_assignee is null then 'shared' else 'individual' end,'messaging_conversation',v_conv.id::text,coalesce(nullif(v_conv.title,''),nullif(v_conv.phone,''),'Диалог'),'messaging_sla_l1',v_event_key)
      on conflict do nothing returning id into v_task_id;
      if v_task_id is not null then
        v_tasks := v_tasks + 1;
        if v_assignee is not null then
          insert into public.crm_task_targets(company_id,task_id,target_type,target_value,target_label)
          select v_conv.company_id,v_task_id,'user',v_assignee::text,coalesce(nullif(u.name,''),nullif(u.email,''),'Ответственный менеджер')
          from public.marketing_users u where u.id=v_assignee on conflict do nothing;
        else
          insert into public.crm_task_targets(company_id,task_id,target_type,target_value,target_label)
          values(v_conv.company_id,v_task_id,'all',null,'Колл-центр') on conflict do nothing;
        end if;
      end if;
      v_task_id := null;
      update public.marketing_conversations set sla_escalation_level=greatest(sla_escalation_level,1),status='PENDING',updated_at=now() where id=v_conv.id;
      v_l1 := v_l1 + 1;
    end if;
    ...
    if now() >= v_conv.awaiting_reply_since + make_interval(mins => v_escalation)
      ...
      from public.crm_company_members m join public.marketing_users u on u.id=m.user_id and u.status='active'
      where m.company_id=v_conv.company_id and m.status='active' and m.role in ('owner','administrator')
      order by case when m.role='owner' then 0 else 1 end,u.last_seen_at desc nulls last,m.user_id limit 1;
      if v_supervisor is null then v_supervisor := v_assignee; end if;

      insert into public.crm_tasks(company_id,title,description,status,stage_key,workflow_key,priority,due_at,sla_minutes,sla_due_at,source,assignment_mode,link_type,link_id,link_label,automation_key,automation_event_key)
      values(v_conv.company_id,'Эскалация SLA: клиент без ответа','Messaging SLA нарушен повторно. Требуется вмешательство руководителя и контроль ответа клиенту.','todo','new','call_center','urgent',now(),5,now()+interval '5 minutes','work_tasks',case when v_supervisor is null then 'shared' else 'individual' end,'messaging_conversation',v_conv.id::text,coalesce(nullif(v_conv.title,''),nullif(v_conv.phone,''),'Диалог'),'messaging_sla_l2',v_event_key)
      on conflict do nothing returning id into v_task_id;
      if v_task_id is not null then
        v_tasks := v_tasks + 1;
        if v_supervisor is not null then
          insert into public.crm_task_targets(company_id,task_id,target_type,target_value,target_label)
          select v_conv.company_id,v_task_id,'user',v_supervisor::text,coalesce(nullif(u.name,''),nullif(u.email,''),'Руководитель')
          from public.marketing_users u where u.id=v_supervisor on conflict do nothing;
        else
          insert into public.crm_task_targets(company_id,task_id,target_type,target_value,target_label)
          values(v_conv.company_id,v_task_id,'all',null,'Руководитель') on conflict do nothing;
        end if;
      end if;
      v_task_id := null;
      update public.marketing_conversations set sla_escalation_level=2,status='PENDING',updated_at=now() where id=v_conv.id;
      v_l2 := v_l2 + 1;
    end if;
  end loop;
  return jsonb_build_object('scanned',v_scanned,'level1',v_l1,'level2',v_l2,'tasksCreated',v_tasks);
end;
$$;
