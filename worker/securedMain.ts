...
function secureEqual(left: string, right: string): boolean { if (left.length !== right.length) return false; let result = 0; for (let i=0;i<left.length;i+=1) result |= left.charCodeAt(i)^right.charCodeAt(i); return result===0; }
function suppliedAdminKey(request: Request): string { const a=request.headers.get('authorization')||''; const bearer=a.toLowerCase().startsWith('bearer ')?a.slice(7).trim():''; return bearer||request.headers.get('x-admin-key')||''; }
function isLegacyAdminRequest(request: Request, env: SecuredEnv): boolean { const supplied=suppliedAdminKey(request); return Boolean(env.FRONTEND_ADMIN_KEY&&supplied&&secureEqual(supplied,env.FRONTEND_ADMIN_KEY)); }
function bypassPermissionBoundary(pathname: string): boolean { return isPublicApiPath(pathname)||pathname==='/api/integrations/meta/callback'||pathname.startsWith('/api/telephony/zadarma/webhook/'); }
function sanitizedRequest(request: Request): Request { const h=new Headers(request.headers); h.delete(INTERNAL_ROLE_HEADER);h.delete(INTERNAL_USER_HEADER);h.delete(INTERNAL_VERIFIED_HEADER);return new Request(request,{headers:h}); }
function trustedRequest(request: Request, role: string, userId: string): Request { const h=new Headers(request.headers);h.delete(INTERNAL_ROLE_HEADER);h.delete(INTERNAL_USER_HEADER);h.delete(INTERNAL_VERIFIED_HEADER);h.set(INTERNAL_ROLE_HEADER,role);h.set(INTERNAL_USER_HEADER,userId);h.set(INTERNAL_VERIFIED_HEADER,'1');return new Request(request,{headers:h}); }

async function scheduleAssignedNotification(request:Request,env:SecuredEnv,ctx:WorkerExecutionContext|undefined,userId:string,task:{id:string;title:string;dueAt?:unknown}):Promise<void>{try{const requestedCompany=(request.headers.get('x-imds-company-id')||'').trim();const companyId=await resolveCompanyId(requestedCompany?{...env,CURRENT_COMPANY_ID:requestedCompany}:env,userId);const notify=notifyAssignedTask(env as unknown as Env,task,companyId).catch((error)=>console.error('Task assigned notification failed',error));if(ctx&&typeof ctx.waitUntil==='function')ctx.waitUntil(notify);else await notify;}catch(error){console.error('Task assigned notification scheduling failed',error);}}

export default {
  async fetch(request: Request, env: SecuredEnv, ctx?: WorkerExecutionContext): Promise<Response> {
    try {
      const url = new URL(request.url); let forwardedRequest: Request | null = null;
      if (url.pathname.startsWith('/api/') && !bypassPermissionBoundary(url.pathname) && !isLegacyAdminRequest(request, env)) {
        ...
        const denied = await authorizeApplicationRequest(request, env, user); if (denied) return denied;
        ...
        if (url.pathname.startsWith('/api/tasks')) {
          if (url.pathname.startsWith('/api/tasks/notifications')) { const response=await handleTaskNotifications(forwardedRequest,env as unknown as Env,url);if(response)return response; }
          if (url.pathname.startsWith('/api/tasks/phase2')) { const response=await handleTaskPhase2(forwardedRequest,env as unknown as Env,url);if(response)return response; }
          if (url.pathname === '/api/tasks/suite/postpone') { const response=await handleTaskQuickActions(forwardedRequest,env as unknown as Env,url);if(response)return response; }
          if (url.pathname.startsWith('/api/tasks/suite')) { const response=await handleTaskSuite(forwardedRequest,env as unknown as Env,url);if(response)return response; }
          const dependencyDenied=await assertTaskDependenciesComplete(forwardedRequest,env as unknown as Env,url);if(dependencyDenied)return dependencyDenied;
          const response=await handleTasks(forwardedRequest,env as unknown as Env,url);
          if(response){if(url.pathname==='/api/tasks'&&request.method==='POST'&&response.ok){const body=await response.clone().json().catch(()=>null) as {task?:{id?:string;title?:string;dueAt?:unknown}}|null;const task=body?.task;if(task?.id&&task.title)await scheduleAssignedNotification(request,env,ctx,user.id,{id:task.id,title:task.title,dueAt:task.dueAt});}return response;}
        }
      }
      if (!forwardedRequest) forwardedRequest = sanitizedRequest(request);
      return app.fetch(forwardedRequest, env, ctx);
    } catch (error) { console.error('Secured worker runtime error', error); return json({ error: error instanceof Error ? error.message : String(error), code: 'WORKER_RUNTIME_ERROR' }, 500); }
  },
