...
async function listConversations(env: MarketingChatEnv): Promise<Response> {
  ...
  const response = await db(env, `marketing_conversations?select=${encodeURIComponent(select)}&archived_at=is.null&order=updated_at.desc&marketing_messages.order=sent_at.desc&marketing_messages.limit=1`);
...
async function listMessages(env: MarketingChatEnv, conversationId: string): Promise<Response> {
  const response = await db(env, `marketing_messages?select=id,conversation_id,body,direction,sender_name,status,sent_at,read_at,metadata&conversation_id=eq.${encodeURIComponent(conversationId)}&order=sent_at.asc`);
  if (!response.ok) return relay(response);
...
async function sendMessage(request: Request, env: MarketingChatEnv, conversationId: string): Promise<Response> {
  ...
  await db(env, `marketing_conversations?id=eq.${encodeURIComponent(conversationId)}`, {
    method: 'PATCH',
...
async function updateConversation(request: Request, env: MarketingChatEnv, conversationId: string): Promise<Response> {
  ...
  const response = await db(env, `marketing_conversations?id=eq.${encodeURIComponent(conversationId)}`, {
    method: 'PATCH',
    headers: { prefer: 'return=representation' },
