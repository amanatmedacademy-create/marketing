...
async function connectedAdvertisingPlatforms(env: LifecycleEnv, companyId: string): Promise<Set<string>> {
  const response = await rest(env, `integration_credentials?company_id=eq.${encodeURIComponent(companyId)}&user_id=is.null&status=eq.connected&provider=in.(meta,tiktok)&select=provider`);
...
async function visibleDailyMetrics(env: LifecycleEnv, companyId: string, url?: URL): Promise<JsonRecord[]> {
  const connected = await connectedAdvertisingPlatforms(env, companyId);
...
async function handleDashboard(env: LifecycleEnv, companyId: string, url: URL): Promise<Response> {
  const grouped = new Map<string, JsonRecord>();
  for (const row of await visibleDailyMetrics(env, companyId, url)) {
...
async function handleSources(env: LifecycleEnv, companyId: string): Promise<Response> {
  const grouped = new Map<string, JsonRecord>();
  for (const row of await visibleDailyMetrics(env, companyId)) {
...
async function handleAds(env: LifecycleEnv, companyId: string): Promise<Response> {
  const connected = await connectedAdvertisingPlatforms(env, companyId);
  if (!connected.size) return json([]);
  const platforms = [...connected].map((item) => `\"${item}\"`).join(',');
  const summary = await rest(env, `marketing_ads_summary?company_id=eq.${encodeURIComponent(companyId)}&select=*&platform=in.(${platforms})&order=revenue.desc`);
  ...
  const fallback = await rest(env, `marketing_ads?company_id=eq.${encodeURIComponent(companyId)}&select=row_key:id,*&platform=in.(${platforms})&order=report_date.desc,revenue.desc`);
...
async function handleCurrencies(env: LifecycleEnv, companyId: string): Promise<Response> {
  const connected = await connectedAdvertisingPlatforms(env, companyId);
  if (!connected.size) return json({ accounts: [] });
  const platforms = [...connected].map((item) => `\"${item}\"`).join(',');
  const response = await rest(env, `marketing_ads?company_id=eq.${encodeURIComponent(companyId)}&select=platform,account_id,account_name,currency&platform=in.(${platforms})&account_id=not.is.null`);
  const rows = await readJson(response) as JsonRecord[];
...
async function disconnectProvider(request: Request, env: LifecycleEnv, url: URL): Promise<Response> {
  ...
  if (provider === 'meta' || provider === 'tiktok') {
    const rpc = await rest(env, 'rpc/manage_ad_provider_data', {
      method: 'POST',
      body: JSON.stringify({ p_company_id: companyId, p_provider: provider, p_purge: purge }),
    });
    if (rpc.ok) {
  ...
  const deleted = await rest(env, `integration_credentials?company_id=eq.${encodeURIComponent(companyId)}&user_id=is.null&provider=eq.${encodeURIComponent(provider)}`, {
    method: 'DELETE',
    headers: { prefer: 'return=minimal' },
  });
  await readJson(deleted);

  return json({ ok: true, companyId, provider, mode: purge ? 'purged' : 'archived', data: dataResult, warning });
}
...
export async function handleIntegrationLifecycle(request: Request, env: LifecycleEnv, url: URL): Promise<Response | null> {
  ...
  if (url.pathname === '/api/dashboard') {
    try {
      return await handleDashboard(env, companyId, url);
    } catch (error) {
      return json({ error: error instanceof Error ? error.message : String(error) }, 400);
    }
  }
  if (url.pathname === '/api/sources') return handleSources(env, companyId);
  if (url.pathname === '/api/ads') return handleAds(env, companyId);
  if (url.pathname === '/api/ads/currencies') return handleCurrencies(env, companyId);
  return null;
}
