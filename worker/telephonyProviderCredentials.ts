...
function webhookUrl(env: UniversalTelephonyEnv, selectedProvider: ConfigurableTelephonyProvider, companyId: string, branchId: string, secret: string): string | null { if (selectedProvider !== 'binotel' && selectedProvider !== 'sipuni') return null; const origin = text((env as unknown as Row).APP_ORIGIN).replace(/\/$/, ''); return origin && secret ? `${origin}/api/telephony/${selectedProvider}/webhook/${companyId}/${branchId}?token=${encodeURIComponent(secret)}` : null; }
...
function summary(row: Row) { return { provider: text(row.provider), branchId: text(row.branch_id) || null, configured: true, status: text(row.status) || 'configured', values: rec(rec(row.config_summary).values), secretFields: rec(rec(row.config_summary).secretFields), lastVerifiedAt: row.last_verified_at || null, lastError: row.last_error || null }; }
...
export async function handleTelephonyProviderConfig(request: Request, env: UniversalTelephonyEnv, url: URL): Promise<Response | null> {
  ...
  if (request.method === 'GET') { if (!existing) return json({ provider: { provider: selectedProvider, branchId, configured: false }, webhookUrl: null }); let secret = ''; if (selectedProvider === 'binotel' || selectedProvider === 'sipuni') try { secret = text((await decrypt(existing, encryptionSecret(env))).webhookSecret); } catch { secret = ''; } return json({ provider: summary(existing), webhookUrl: webhookUrl(env, selectedProvider, companyId, branchId, secret) }); }
  ...
  return json({ ok: true, provider: summary(saved[0] || stored), active: incoming.activate === true, webhookUrl: webhookUrl(env, selectedProvider, companyId, branchId, text(payload.webhookSecret)) });
}
