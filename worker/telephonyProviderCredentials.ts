...
export async function loadTelephonyProviderCredential(env: UniversalTelephonyEnv, selectedProvider: ConfigurableTelephonyProvider, explicitCompanyId?: string): Promise<{ row: Row; payload: Row } | null> { const companyId = explicitCompanyId || requireCompanyId(env); const found = (await db<Row[]>(env, `integration_credentials?company_id=eq.${encodeURIComponent(companyId)}&user_id=is.null&provider=eq.${selectedProvider}&select=*&limit=1`))[0]; if (!found) return null; return { row: found, payload: await decrypt(found, encryptionSecret(env)) }; }

export async function markTelephonyProviderStatus(env: UniversalTelephonyEnv, companyId: string, selectedProvider: ConfigurableTelephonyProvider, ok: boolean, error?: unknown): Promise<void> { const now = new Date().toISOString(); await db(env, `integration_credentials?company_id=eq.${encodeURIComponent(companyId)}&user_id=is.null&provider=eq.${selectedProvider}`, { method: 'PATCH', headers: { prefer: 'return=minimal' }, body: JSON.stringify({ status: ok ? 'connected' : 'error', last_verified_at: now, last_error: ok ? null : (error instanceof Error ? error.message : String(error || 'Telephony provider error')).slice(0, 1000), updated_at: now }) }); }
...
export async function handleTelephonyProviderConfig(request: Request, env: UniversalTelephonyEnv, url: URL): Promise<Response | null> {
  ...
  if (request.method === 'GET') {
    if (!existing) return json({ provider: { provider: selectedProvider, configured: false }, webhookUrl: null });
    let secret = '';
    if (selectedProvider === 'binotel' || selectedProvider === 'sipuni') try { secret = text((await decrypt(existing, encryptionSecret(env))).webhookSecret); } catch { secret = ''; }
    return json({ provider: summary(existing), webhookUrl: webhookUrl(env, selectedProvider, companyId, secret) });
  }
  if (request.method === 'DELETE') { await db(env, `integration_credentials?company_id=eq.${encodeURIComponent(companyId)}&user_id=is.null&provider=eq.${selectedProvider}`, { method: 'DELETE', headers: { prefer: 'return=minimal' } }); return json({ ok: true, provider: selectedProvider }); }
  ...
  const stored = { provider: selectedProvider, company_id: companyId, user_id: null, ...sealed, config_summary: { values, secretFields }, status: existing?.status === 'connected' ? 'connected' : 'configured', last_error: null, updated_at: new Date().toISOString() };
  ...
  if (incoming.activate === true) await db(env, `telephony_settings?company_id=eq.${encodeURIComponent(companyId)}`, { method: 'PATCH', body: JSON.stringify({ provider: selectedProvider, updated_at: new Date().toISOString() }) });
  return json({ ok: true, provider: summary(saved[0] || stored), active: incoming.activate === true, webhookUrl: webhookUrl(env, selectedProvider, companyId, text(payload.webhookSecret)) });
}
