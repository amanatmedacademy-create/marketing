...
function dbHeaders(env: Env, init: RequestInit): Headers {
  ...
  if (init.body != null) result.set('Content-Type', 'application/json');
  if (init.method && !['GET', 'HEAD'].includes(init.method) && !result.has('Prefer')) result.set('Prefer', 'return=representation');
  return result;
}
async function db<T>(env: Env, path: string, init: RequestInit = {}): Promise<T> {
  const response = await fetch(`${env.SUPABASE_URL.replace(/\/+$/, '')}/rest/v1${path}`, { ...init, headers: dbHeaders(env, init), cache: 'no-store' });
  if (!response.ok) throw new UpstreamError(response.status, (await response.text().catch(() => '')).slice(0, 1500));
...
async function workspace(request: Request, env: Env, url: URL, requestId: string): Promise<Response> {
  ...
  const pipelines = await db<Row[]>(env, `/crm_pipelines?select=${PIPELINE_SELECT}&company_id=eq.${companyId}&order=position.asc,created_at.asc&limit=100`);
  const stages = await db<Row[]>(env, `/crm_pipeline_stages?select=${STAGE_SELECT}&company_id=eq.${companyId}&order=pipeline_id.asc,position.asc&limit=1000`);
  ...
  const params = new URLSearchParams({ select: DEAL_SELECT, company_id: `eq.${companyId}`, deleted_at: 'is.null', order: 'stage_id.asc,position.asc,updated_at.desc', limit: '2000' });
  ...
  const weightedAmount = deals.reduce((sum, deal) => {
    const stage = selectedStages.find((item) => text(item, 'id') === text(deal, 'stage_id'));
...
async function getPipelineAndStage(env: Env, companyId: string, pipelineId?: string, stageId?: string): Promise<{ pipelineId: string; stageId: string }> {
...
async function createPipeline(request: Request, env: Env, requestId: string): Promise<Response> {
  ...
  if (body?.isDefault) await db<Row[]>(env, `/crm_pipelines?company_id=eq.${companyId}&is_default=eq.true`, { method: 'PATCH', body: JSON.stringify({ is_default: false, updated_at: new Date().toISOString() }) });
  const rows = await db<Row[]>(env, `/crm_pipelines?select=${PIPELINE_SELECT}`, { method: 'POST', body: JSON.stringify({ company_id: companyId, name, is_default: body?.isDefault === true, position: Date.now(), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }) });
  ...
  ].map((stage) => ({ ...stage, company_id: companyId, pipeline_id: text(pipeline, 'id') }));
...
async function updatePipeline(request: Request, env: Env, requestId: string, id: string): Promise<Response> {
  ...
  if (body.isDefault === true) {
    await db<Row[]>(env, `/crm_pipelines?company_id=eq.${companyId}&is_default=eq.true&id=neq.${id}`, { method: 'PATCH', body: JSON.stringify({ is_default: false, updated_at: new Date().toISOString() }) });
    patch.is_default = true;
  }
  const rows = await db<Row[]>(env, `/crm_pipelines?id=eq.${id}&company_id=eq.${companyId}&select=${PIPELINE_SELECT}`, { method: 'PATCH', body: JSON.stringify(patch) });
...
async function deletePipeline(request: Request, env: Env, requestId: string, id: string): Promise<Response> {
  if (!canManage(request)) return json(requestId, { error: 'Управление воронками доступно администратору' }, 403);
  const companyId = await resolveCompanyId(env, request);
  const deals = await db<Row[]>(env, `/crm_deals?select=id&company_id=eq.${companyId}&pipeline_id=eq.${id}&deleted_at=is.null&limit=1`);
  if (deals.length) return json(requestId, { error: 'Нельзя удалить воронку со сделками. Сначала перенесите сделки.' }, 409);
  const pipelines = await db<Row[]>(env, `/crm_pipelines?select=id&company_id=eq.${companyId}&limit=2`);
  if (pipelines.length <= 1) return json(requestId, { error: 'Нельзя удалить единственную воронку' }, 409);
  await db<void>(env, `/crm_pipelines?id=eq.${id}&company_id=eq.${companyId}`, { method: 'DELETE' });
...
async function updateStage(request: Request, env: Env, requestId: string, id: string): Promise<Response> {
  ...
  if (body.position !== undefined) patch.position = Math.max(0, Math.trunc(body.position));
  const rows = await db<Row[]>(env, `/crm_pipeline_stages?id=eq.${id}&company_id=eq.${companyId}&select=${STAGE_SELECT}`, { method: 'PATCH', body: JSON.stringify(patch) });
...
async function deleteStage(request: Request, env: Env, requestId: string, id: string): Promise<Response> {
  if (!canManage(request)) return json(requestId, { error: 'Управление стадиями доступно администратору' }, 403);
  const companyId = await resolveCompanyId(env, request);
  const deals = await db<Row[]>(env, `/crm_deals?select=id&company_id=eq.${companyId}&stage_id=eq.${id}&deleted_at=is.null&limit=1`);
  if (deals.length) return json(requestId, { error: 'Нельзя удалить стадию со сделками. Сначала перенесите карточки.' }, 409);
  const stageRows = await db<Row[]>(env, `/crm_pipeline_stages?select=pipeline_id&company_id=eq.${companyId}&id=eq.${id}&limit=1`);
  if (!stageRows[0]) return json(requestId, { error: 'Стадия не найдена' }, 404);
  const siblings = await db<Row[]>(env, `/crm_pipeline_stages?select=id&company_id=eq.${companyId}&pipeline_id=eq.${text(stageRows[0], 'pipeline_id')}&limit=2`);
  if (siblings.length <= 1) return json(requestId, { error: 'В воронке должна остаться хотя бы одна стадия' }, 409);
  await db<void>(env, `/crm_pipeline_stages?id=eq.${id}&company_id=eq.${companyId}`, { method: 'DELETE' });
  return new Response(null, { status: 204, headers: headers(requestId) });
}
...
export async function handleSalesFunnelV2(request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  try {
    ...
    if (error instanceof UpstreamError) {
      console.error('Sales funnel upstream error', error.status, error.detail);
      return json(requestId, { error: error.status === 409 ? 'Конфликт данных воронки' : 'Ошибка базы данных воронки', requestId }, error.status >= 400 && error.status < 500 ? error.status : 502);
    }
