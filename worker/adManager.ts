...
import { syncMetaReach } from './metaReachSync';
...
export async function handleAdManager(_request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  try {
    await syncMetaReach(env, Math.max(days, 90));
  } catch (error) {
    syncWarning = error instanceof Error ? error.message : String(error);
    console.error('Automatic Meta reach sync failed', error);
  }
  ...
  return new Response(JSON.stringify({ period: { from, to, days }, accounts, rows: result, syncWarning }), {
