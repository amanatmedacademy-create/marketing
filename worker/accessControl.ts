...
function headers(env: AccessControlEnv): HeadersInit { return { apikey: env.SUPABASE_SERVICE_ROLE_KEY, authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`, accept: 'application/json' }; }
async function db<T>(env: AccessControlEnv, path: string): Promise<T> { const response = await fetch(`${env.SUPABASE_URL.replace(/\/$/, '')}/rest/v1/${path}`, { headers: headers(env) }); const body = await response.text(); if (!response.ok) throw new Error(`Access control ${response.status}: ${body.slice(0, 700)}`); return (body ? JSON.parse(body) : null) as T; }
...
export async function resolveUserAccess(env: AccessControlEnv, userId: string, role?: string): Promise<{ companyId: string; positionId: string | null; jobTitle: string | null; permissions: AccessMap }> {
  ...
  if (role === 'administrator') { for (const grant of Object.values(permissions)) for (const action of ACTIONS) grant[action] = true; return { companyId, positionId: null, jobTitle: 'Администратор системы', permissions }; }
  ...
  if (positionId) { const rows = await db<Row[]>(env, `crm_access_position_permissions?company_id=eq.${encodeURIComponent(companyId)}&position_id=eq.${encodeURIComponent(positionId)}&select=*`); for (const row of rows) { const moduleId = text(row.module_id); if (!permissions[moduleId]) permissions[moduleId] = emptyGrant(); for (const action of ACTIONS) permissions[moduleId][action] = bool(row[`can_${action}`]); } }
  ...
  for (const row of overrides) { const moduleId = text(row.module_id); if (!permissions[moduleId]) permissions[moduleId] = emptyGrant(); for (const action of ACTIONS) { const value = row[`can_${action}`]; if (typeof value === 'boolean') permissions[moduleId][action] = value; } }
...
const routeRules: Array<{ test: (path: string) => boolean; moduleId: string }> = [
  ...
  { test: (p) => p.startsWith('/api/leads'), moduleId: 'crm.leads' },
...
export function permissionForRequest(pathname: string, method: string): { moduleId: string; action: AccessAction } | null { const rule = routeRules.find((item) => item.test(pathname)); if (!rule) return null; const normalized = method.toUpperCase(); const action: AccessAction = normalized === 'GET' || normalized === 'HEAD' ? 'view' : normalized === 'POST' ? 'create' : normalized === 'PATCH' || normalized === 'PUT' ? 'edit' : normalized === 'DELETE' ? 'delete' : 'view'; return { moduleId: rule.moduleId, action }; }
export function hasPermission(access: AccessMap, moduleId: string, action: AccessAction): boolean { const grant = access[moduleId]; return Boolean(grant && (grant[action] || grant.manage)); }
