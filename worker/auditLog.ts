...
const companyId = (env: Env) => requireCompanyId(env as ScopedEnv);
...
function requestRole(request: Request): string { return (request.headers.get(ROLE_HEADER) || '').trim().toLowerCase(); }
...
export async function recordErrorEvent(env: Env, input: {
  ...
  try {
    ...
    const existing = await db<Row[]>(env, `/error_logs?select=id,repeat_count,status&company_id=eq.${encodeURIComponent(tenantId)}&fingerprint=eq.${fingerprint}&limit=1`);
    if (existing[0]) {
      const repeat = Number(existing[0].repeat_count || 0) + 1;
      const status = existing[0].status === 'RESOLVED' ? 'OPEN' : existing[0].status;
      await db(env, `/error_logs?company_id=eq.${encodeURIComponent(tenantId)}&fingerprint=eq.${fingerprint}`, {
      ...
      await db(env, '/error_logs', {
        ...
        body: JSON.stringify({
          company_id: tenantId,
          source: input.source.slice(0, 80), endpoint: input.endpoint.slice(0, 300), code: input.code.slice(0, 40), message,
          correlation_id: input.correlationId || null, fingerprint,
...
export function planAudit(method: string, path: string, body: Row | null): AuditPlan | null {
  ...
  if (/^\/api\/(auth\/)?users\/[^/]+$/.test(path) && ['PATCH', 'PUT'].includes(method) && body && 'role' in body) return { action: 'user.role_changed', entityType: 'user', entityId: segment(2), captureBody: true };
  ...
  if (path.startsWith('/api/integrations/') && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) return { action: method === 'DELETE' ? 'integration.deleted' : 'integration.changed', entityType: 'integration', entityId: segment(2), captureBody: true };
  const funnelLead = path.match(/^\/api\/funnel\/leads\/([^/]+)$/);
  if (funnelLead && method === 'PATCH') return { action: body && typeof body.stage === 'string' ? 'funnel.stage_changed' : 'funnel.lead_updated', entityType: 'funnel_lead', entityId: funnelLead[1], captureBody: true };
  ...
  if (chatThread && method === 'PATCH') return { action: body && typeof body.status === 'string' ? 'chat.stage_changed' : 'chat.thread_updated', entityType: 'chat_thread', entityId: chatThread[1], captureBody: true };
  if (method === 'DELETE' && path.startsWith('/api/')) return { action: 'entity.deleted', entityType: segment(1) || 'api', entityId: segment(2), captureBody: false };
  if (method === 'GET' && (path.includes('/export') || path.endsWith('.csv'))) return { action: 'data.exported', entityType: segment(1) || 'api', entityId: null, captureBody: false };
  return null;
}
...
function mapErrorRow(row: Row) {
  return { id: String(row.id || ''), source: String(row.source || ''), endpoint: String(row.endpoint || ''), code: String(row.code || ''), message: String(row.message || ''), correlationId: row.correlation_id ? String(row.correlation_id) : undefined, repeatCount: Number(row.repeat_count || 0), retryAttempts: Number(row.retry_attempts || 0), firstSeenAt: String(row.first_seen_at || ''), lastSeenAt: String(row.last_seen_at || ''), status: String(row.status || 'OPEN'), metadata: row.metadata && typeof row.metadata === 'object' ? row.metadata : {} };
}
function mapAuditRow(row: Row) {
  return { id: String(row.id || ''), userId: row.user_id ? String(row.user_id) : undefined, action: String(row.action || ''), entityType: row.entity_type ? String(row.entity_type) : undefined, entityId: row.entity_id ? String(row.entity_id) : undefined, before: row.before ?? null, after: row.after ?? null, ip: row.ip ? String(row.ip) : undefined, userAgent: row.user_agent ? String(row.user_agent) : undefined, correlationId: row.correlation_id ? String(row.correlation_id) : undefined, createdAt: String(row.created_at || '') };
}
...
export async function handleAuditApi(request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  try {
    ...
    if (request.method === 'GET' && path === '/api/audit') {
      ...
      const params = new URLSearchParams({ select: '*', company_id: `eq.${tenantId}`, order: 'created_at.desc', limit: String(limit) });
    ...
    if (request.method === 'POST' && path === '/api/audit/events') {
      ...
      await recordAudit(env, { userId: requestUserId(request), action, entityType: typeof body?.entityType === 'string' ? body.entityType.slice(0, 80) : null, entityId: typeof body?.entityId === 'string' ? body.entityId : null, after: body?.details ?? null, ip, userAgent, correlationId: correlationId(request) });
      return json({ ok: true }, 201);
    }

    if (request.method === 'GET' && path === '/api/errors') {
      if (!canOperate) return json({ error: 'Недостаточно прав' }, 403);
      const params = new URLSearchParams({ select: '*', company_id: `eq.${tenantId}`, order: 'last_seen_at.desc', limit: '200' });
    ...
    if (statusMatch && request.method === 'PATCH') {
      ...
      const rows = await db<Row[]>(env, `/error_logs?id=eq.${encodeURIComponent(statusMatch[1])}&company_id=eq.${encodeURIComponent(tenantId)}&select=*`, { method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ status, updated_at: new Date().toISOString() }) });
      if (!rows[0]) return json({ error: 'Ошибка не найдена' }, 404);
      await recordAudit(env, { userId: requestUserId(request), action: 'error.status_changed', entityType: 'error_log', entityId: statusMatch[1], after: { status }, ...requestClient(request), correlationId: correlationId(request) });
    ...
    if (retryMatch && request.method === 'POST') {
      if (!canOperate) return json({ error: 'Недостаточно прав' }, 403);
      const rows = await db<Row[]>(env, `/error_logs?id=eq.${encodeURIComponent(retryMatch[1])}&company_id=eq.${encodeURIComponent(tenantId)}&select=*&limit=1`);
      const row = rows[0];
      if (!row) return json({ error: 'Ошибка не найдена' }, 404);
      const updated = await db<Row[]>(env, `/error_logs?id=eq.${encodeURIComponent(retryMatch[1])}&company_id=eq.${encodeURIComponent(tenantId)}&select=*`, { method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ status: 'RETRYING', retry_attempts: Number(row.retry_attempts || 0) + 1, updated_at: new Date().toISOString() }) });
      const metadata = row.metadata && typeof row.metadata === 'object' ? row.metadata as Row : {};
      await recordAudit(env, { userId: requestUserId(request), action: 'error.retry_requested', entityType: 'error_log', entityId: retryMatch[1], after: { endpoint: row.endpoint, code: row.code }, ...requestClient(request), correlationId: correlationId(request) });
      return json({ ...mapErrorRow(updated[0] || row), retryPath: typeof metadata.retryPath === 'string' ? metadata.retryPath : (String(row.source) === 'integration.sync' ? '/api/integrations/sync' : undefined), retryMethod: typeof metadata.retryMethod === 'string' ? metadata.retryMethod : 'POST' });
    }
