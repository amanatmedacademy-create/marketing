...
function mapCustomField(row: Row) {
  return { id: asString(row.id), key: asString(row.field_key), label: asString(row.label), type: asString(row.field_type) as CustomFieldType, options: arrayStrings(row.options), required: bool(row.is_required), active: row.is_active !== false, position: numberValue(row.position), createdAt: asString(row.created_at), updatedAt: asString(row.updated_at) };
}
...
async function loadCustomFields(request: Request, env: Env, companyId: string, dealId: string) {
  ...
  const activeFilter = canManageFields ? '' : '&is_active=eq.true';
  const [definitions, values] = await Promise.all([
    supabase<Row[]>(env, `/crm_custom_field_definitions?select=id,field_key,label,field_type,options,is_required,is_active,position,created_at,updated_at&company_id=eq.${encodeEq(companyId)}&entity_type=eq.deal${activeFilter}&order=position.asc,created_at.asc`),
...
function customFieldPatch(body: Row): Row {
  ...
  if ('type' in body) { const type = asString(body.type) as CustomFieldType; if (!FIELD_TYPES.has(type)) throw new Error('Некорректный тип поля'); patch.field_type = type; patch.options = type === 'select' ? arrayStrings(body.options).slice(0, 50).map((value) => value.slice(0, 80)) : []; if (type === 'select' && !(patch.options as string[]).length) throw new Error('Для списка добавьте хотя бы один вариант'); }
  else if ('options' in body) patch.options = arrayStrings(body.options).slice(0, 50).map((value) => value.slice(0, 80));
...
async function updateCustomField(request: Request, env: Env, deal: Row, fieldId: string, id: string): Promise<Response> {
  ...
  const body = await readBody(request); let patch: Row;
  try { patch = customFieldPatch(body); } catch (error) { return json(id, { error: error instanceof Error ? error.message : 'Некорректные настройки поля', requestId: id }, 400); }
  ...
  if (!rows[0]) return json(id, { error: 'Поле не найдено', requestId: id }, 404);
  return json(id, mapCustomField(rows[0]));
}
async function saveCustomValues(request: Request, env: Env, deal: Row, id: string): Promise<Response> {
  ...
  const definitions = await supabase<Row[]>(env, `/crm_custom_field_definitions?select=id,label,field_type,options,is_required&company_id=eq.${encodeEq(companyId)}&entity_type=eq.deal&is_active=eq.true`); const definitionMap = new Map(definitions.map((item) => [asString(item.id), item])); const saved: Record<string, unknown> = {};
  for (const [fieldId, raw] of Object.entries(values)) { const definition = definitionMap.get(fieldId); if (!definition) continue; const value = normalizeCustomValue(definition, raw); const rows = await supabase<Row[]>(env, `/crm_custom_field_values?on_conflict=company_id,deal_id,field_id`, { method: 'POST', headers: { prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify({ company_id: companyId, deal_id: dealId, field_id: fieldId, value, updated_by: actorId(request), updated_at: new Date().toISOString() }) }); saved[fieldId] = rows[0]?.value ?? value; }
...
export async function handleDealWorkspace(request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  const id = requestId(request); const base = url.pathname.match(/^\/api\/deal-workspace\/([0-9a-f-]{36})(?:\/(activities|custom-fields|custom-values)(?:\/([0-9a-f-]{36}))?)?\/?$/i);
