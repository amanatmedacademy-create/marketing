...
function inFilter(values: string[]): string {
  return values.map((value) => value.replace(/[^0-9a-f-]/gi, '')).filter(Boolean).join(',');
}
...
function mapActivity(row: Row) {
  return {
    ...
    type: asString(row.activity_type),
...
function mapMessage(row: Row) {
  return {
    ...
    attachmentName: asNullableString(row.attachment_name),
    attachmentMimeType: asNullableString(row.attachment_mime_type),
  };
}
...
function mapCall(row: Row) {
  return {
    ...
    operatorName: asNullableString(row.operator_name) || asNullableString(row.client_name),
    clientPhone: asNullableString(row.client_phone),
    channel: asNullableString(row.channel),
    status: asString(row.call_status),
    startedAt: asString(row.started_at),
    scheduledAt: asNullableString(row.scheduled_at),
    durationSeconds: Number(row.duration_seconds || 0),
    recordingUrl: asNullableString(row.recording_url),
    summary: asNullableString(row.summary),
    result: asNullableString(row.call_result),
...
function mapStageEvent(row: Row) {
  return {
    ...
    fromStageId: asNullableString(row.from_stage_id),
    toStageId: asString(row.to_stage_id),
    actorUserId: asNullableString(row.actor_user_id),
...
function mapConversation(row: Row) {
  return {
    ...
    status: asString(row.status),
    assignedUserId: asNullableString(row.assigned_user_id),
...
function mapUser(row: Row) {
  return {
    id: asString(row.id),
    fullName: asNullableString(row.full_name) || asNullableString(row.name) || 'Пользователь',
  };
}
...
async function getDeal(env: Env, dealId: string): Promise<Row | null> {
  const rows = await supabase<Row[]>(env, `/crm_deals?select=id,company_id,marketing_lead_id,phone,pipeline_id,stage_id&deleted_at=is.null&id=eq.${encodeEq(dealId)}&limit=1`);
  return rows[0] || null;
}
...
async function getConversations(env: Env, companyId: string, leadId: string | null, phone: string | null): Promise<Row[]> {
  ...
  if (phone) {
    return supabase<Row[]>(env, `/marketing_conversations?select=id,lead_id,title,phone,channel,status,assigned_user_id,unread_count,last_message_at&company_id=eq.${encodeEq(companyId)}&phone=eq.${encodeEq(phone)}&archived_at=is.null&order=last_message_at.desc.nullslast&limit=20`);
  }
  return [];
}
...
async function getCalls(env: Env, companyId: string, leadId: string | null, phone: string | null): Promise<Row[]> {
  ...
  if (phone) {
    return supabase<Row[]>(env, `/marketing_calls?select=id,lead_id,operator_name,client_name,client_phone,channel,call_status,started_at,scheduled_at,duration_seconds,recording_url,summary,call_result,next_action&company_id=eq.${encodeEq(companyId)}&client_phone=eq.${encodeEq(phone)}&order=started_at.desc&limit=100`);
  }
  return [];
}
...
async function loadWorkspace(env: Env, deal: Row) {
  ...
  const [activities, stageEvents, calls, messages] = await Promise.all([
    ...
    conversationIds.length
      ? supabase<Row[]>(env, `/marketing_messages?select=id,conversation_id,body,direction,sender_name,status,sent_at,attachment_name,attachment_mime_type&company_id=eq.${encodeEq(companyId)}&conversation_id=in.(${inFilter(conversationIds)})&order=sent_at.desc&limit=200`)
      : Promise.resolve([]),
  ]);
  ...
  const users = actorIds.length
    ? await supabase<Row[]>(env, `/marketing_users?select=id,name,full_name&id=in.(${inFilter(actorIds)})`)
    : [];
...
export async function handleDealWorkspace(request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  try {
    ...
    if (!deal) return json(id, { error: 'Сделка не найдена', requestId: id }, 404);

    if (request.method === 'GET' && !activityId) {
      return json(id, await loadWorkspace(env, deal));
    }

    if (!WRITE_ROLES.has(role(request))) {
      return json(id, { error: 'Недостаточно прав для изменения карточки', requestId: id }, 403);
    }
    ...
    if (request.method === 'POST' && url.pathname.endsWith('/activities')) {
      ...
      const rows = await supabase<Row[]>(env, '/crm_deal_activities', {
        ...
        body: JSON.stringify({
          ...
          due_at: type === 'task' ? asNullableString(body.dueAt) : null,
          actor_user_id: actorId(request),
        }),
      });
    ...
    if (request.method === 'PATCH' && activityId) {
      ...
      const rows = await supabase<Row[]>(env, `/crm_deal_activities?id=eq.${encodeEq(activityId)}&deal_id=eq.${encodeEq(dealId)}&company_id=eq.${encodeEq(asString(deal.company_id))}`, {
        method: 'PATCH',
        headers: { prefer: 'return=representation' },
    ...
    if (error instanceof UpstreamError) {
      return json(id, { error: 'Ошибка источника данных карточки', detail: error.detail, requestId: id }, 502);
    }
