...
async function exchangeForLongToken(env: MetaSdkEnv, shortToken: string): Promise<string> {
  ...
  const params = new URLSearchParams({ grant_type: 'fb_exchange_token', client_id: env.META_APP_ID, client_secret: env.META_APP_SECRET, fb_exchange_token: shortToken });
...
async function supabaseWrite(env: MetaSdkEnv, path: string, rows: unknown, prefer: string): Promise<void> {
  const response = await fetch(`${env.SUPABASE_URL.replace(/\/$/, '')}/rest/v1/${path}`, { method: 'POST', headers: supabaseHeaders(env, prefer), body: JSON.stringify(rows) });
...
async function save(env: MetaSdkEnv, accessToken: string, items: MetaAccount[]): Promise<void> {
  ...
  const payload = {
    provider: 'meta', user_id: null, encrypted_payload: encrypted.encryptedPayload, iv: encrypted.iv,
    config_summary: {
      values: {
        adAccountIds: ids.join(','), graphVersion: graphVersion(env),
    ...
    }, status: 'connected', last_error: null, last_verified_at: new Date().toISOString(),
  };
  const base = env.SUPABASE_URL.replace(/\/$/, '');
  const updateResponse = await fetch(`${base}/rest/v1/integration_credentials?provider=eq.meta&user_id=is.null`, { method: 'PATCH', headers: supabaseHeaders(env, 'return=representation'), body: JSON.stringify(payload) });
  ...
  const insertResponse = await fetch(`${base}/rest/v1/integration_credentials`, { method: 'POST', headers: supabaseHeaders(env, 'return=minimal'), body: JSON.stringify(payload) });
...
function actionValue(items: unknown, accepted: string[]): number {
  ...
  return items.reduce((sum, item) => { const row = record(item); return types.has(text(row.action_type)) ? sum + number(row.value) : sum; }, 0);
}
const resultActions = [
  'lead','onsite_conversion.lead_grouped','offsite_conversion.fb_pixel_lead',
  'onsite_conversion.messaging_conversation_started_7d','onsite_conversion.messaging_conversation_started',
  'onsite_conversion.messaging_first_reply','onsite_conversion.messaging_conversation_replied_7d',
];
...
async function syncInsights(env: MetaSdkEnv, accessToken: string, items: MetaAccount[], days = 90): Promise<number> {
  ...
  for (const account of items) {
    ...
    const insightParams = new URLSearchParams({ access_token: accessToken, level: 'ad', fields, time_increment: '1', time_range: JSON.stringify(range), limit: '500' });
    ...
    for (const item of insights) {
      ...
      rows.push({
        external_id: `meta:${accountId}:${adId}`, report_date: text(item.date_start) || range.until,
        source: 'Meta', platform: 'Meta', account_id: accountId, account_name: text(item.account_name) || account.name || accountId,
        account_status: text(account.account_status), currency: account.currency || 'USD', account_timezone: account.timezone_name || null,
        campaign_id: text(item.campaign_id || campaign.id), campaign_name: text(item.campaign_name || campaign.name) || 'Meta campaign',
        adset_id: text(item.adset_id || adset.id), adset_name: text(item.adset_name || adset.name), ad_id: adId,
        creative_name: text(item.ad_name || ad.name), creative_type: null, status: effectiveStatus, effective_status: effectiveStatus,
        ...
        leads: results, results, result_indicator: results ? 'result' : null, cost_per_result: results ? number(item.spend) / results : 0,
        ...
        utm_source: 'meta', utm_medium: 'paid_social', utm_campaign: text(item.campaign_id), utm_content: adId,
    ...
    for (const ad of ads) {
      ...
      const effectiveStatus = text(ad.effective_status || ad.status) || 'UNKNOWN';
      rows.push({
        external_id: `meta:${accountId}:${adId}`, report_date: range.until,
        source: 'Meta', platform: 'Meta', account_id: accountId, account_name: account.name || accountId,
        account_status: text(account.account_status), currency: account.currency || 'USD', account_timezone: account.timezone_name || null,
        campaign_id: text(campaign.id), campaign_name: text(campaign.name) || 'Meta campaign',
        adset_id: text(adset.id), adset_name: text(adset.name), ad_id: adId, creative_name: text(ad.name),
        status: effectiveStatus, effective_status: effectiveStatus, impressions: 0, reach: 0, clicks: 0, link_clicks: 0, spend: 0,
        leads: 0, results: 0, target_leads: 0, arrived: 0, sales: 0, revenue: 0,
        utm_source: 'meta', utm_medium: 'paid_social', utm_campaign: text(campaign.id), utm_content: adId,
