...
type Payload = { range: { since: string; until: string }; currency: string; source: 'canonical' | 'legacy_meta'; totals: Totals; metrics: Metrics; previousTotals: Totals; accounts: Account[]; campaigns: Campaign[]; trend: Array<{ date: string; spend: number; leads: number }>; freshness: { lastMetricDate: string | null; lastSyncedAt: string | null } };
...
export function MetaAdsAllAccounts() {
  ...
  return <div style={{ display: 'grid', gap: 16 }}>
    <section className="ads-card" style={{ display: 'grid', gap: 16 }}>
      ...
      <div className="ads-metrics"><article><span>Расход</span><strong>{loading ? '—' : money(totals?.spend ?? 0, data?.currency)}</strong></article><article><span>Показы</span><strong>{loading ? '—' : num.format(totals?.impressions ?? 0)}</strong></article><article><span>Клики</span><strong>{loading ? '—' : num.format((totals?.linkClicks || totals?.clicks) ?? 0)}</strong><small>CTR {pct(data?.metrics.ctr ?? null)}</small></article><article><span>CPC</span><strong>{loading ? '—' : money(data?.metrics.cpc ?? null, data?.currency)}</strong></article><article><span>Лиды</span><strong>{loading ? '—' : num.format(totals?.leads ?? 0)}</strong><small>CPL {money(data?.metrics.cpl ?? null, data?.currency)}</small></article></div>
      {!!data?.trend.length && <div style={{ display: 'flex', alignItems: 'end', gap: 4, minHeight: 130 }}>{data.trend.map((item) => <div key={item.date} title={`${item.date}: ${money(item.spend, data.currency)} · ${item.leads} лидов`} style={{ flex: 1, minWidth: 4, height: 120, display: 'grid', alignItems: 'end' }}><div style={{ height: `${Math.max(3, item.spend / maxSpend * 100)}%`, background: 'linear-gradient(180deg,#1877F2,#E1306C)', borderRadius: '5px 5px 0 0' }} /></div>)}</div>}
    </section>

    <section className="ads-card"><div className="ads-card-toolbar" style={{ justifyContent: 'space-between' }}><strong>Кампании ({rows.length})</strong><label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск кампании" /></label></div><div style={{ overflowX: 'auto' }}><table className="ads-performance-table"><thead><tr><th>Кампания</th><th>Расход</th><th>Показы</th><th>Клики</th><th>Лиды</th><th>CTR</th><th>CPC</th><th>CPL</th></tr></thead><tbody>{rows.map((item) => { const key = `${item.accountId}-${item.campaignId}`; return <Fragment key={key}><tr><td><button onClick={() => toggle(key)}>{expanded.has(key) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button><strong>{item.campaignName}</strong><small style={{ marginLeft: 8 }}>{item.accountName} · {item.status}</small></td><td>{money(item.totals.spend,item.currency)}</td><td>{num.format(item.totals.impressions)}</td><td>{num.format(item.totals.linkClicks || item.totals.clicks)}</td><td>{num.format(item.totals.leads)}</td><td>{pct(item.metrics.ctr)}</td><td>{money(item.metrics.cpc,item.currency)}</td><td>{money(item.metrics.cpl,item.currency)}</td></tr>{expanded.has(key) && item.adGroups.map((group) => <tr key={`${key}-${group.id}`}><td style={{ paddingLeft: 48 }}>↳ {group.name}</td><td>{money(group.totals.spend,item.currency)}</td><td>{num.format(group.totals.impressions)}</td><td>{num.format(group.totals.linkClicks || group.totals.clicks)}</td><td>{num.format(group.totals.leads)}</td><td>{pct(group.metrics.ctr)}</td><td>{money(group.metrics.cpc,item.currency)}</td><td>{money(group.metrics.cpl,item.currency)}</td></tr>)}</Fragment>; })}</tbody></table></div></section>
  </div>;
}
