...
export function CreateLeadModal({ pipeline, isSubmitting, onClose, onSubmit }: Props) {
  ...
  return <div className="lead-modal-backdrop" onMouseDown={onClose}>
    <form className="lead-modal" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <footer><button type="button" onClick={onClose}>Отмена</button><button className="primary" disabled={!name.trim() || !stageId || isSubmitting}>{isSubmitting ? 'Создание…' : 'Создать лид'}</button></footer>
    </form>
  </div>;
}
