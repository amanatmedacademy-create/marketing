...
const RELEASE = 'server-entitlement-enforcement-v1';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (requiredModule) {
        try {
          const allowed = await hasModuleEntitlement(env, session, requiredModule);
          if (!allowed) return apiError(403, 'MODULE_NOT_ENTITLED', `Модуль ${requiredModule} не подключён для компании`);
        } catch (error) {
          return apiError(503, 'ENTITLEMENT_CHECK_FAILED', error instanceof Error ? error.message : 'Не удалось проверить права модуля');
        }
      }
