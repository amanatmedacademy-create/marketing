...
import { BarChart3, Bell, Cable, ChartNoAxesCombined, FileText, History, LayoutDashboard, Menu, MessageCircle, MessageSquareText, PhoneCall, Search, ServerCog, Settings, ShieldCheck, Tags, TriangleAlert, UsersRound, Workflow } from 'lucide-react';
...
import { LeadsPage as UnifiedLeadsPage } from './pages/LeadsPage';
...
function Shell() {
  ...
  return <div className="marketing-shell">
    ...
    <main><header className="marketing-topbar"><button className="marketing-menu" type="button" onClick={() => setOpen(!open)}><Menu size={21}/></button><div className="marketing-search"><Search size={17}/><input placeholder="Поиск лидов, кампаний, каналов и UTM"/></div><div className="marketing-top-actions"><button type="button" aria-label="Уведомления" onClick={() => setWorkspace('profile')}><Bell size={18}/></button>{user.role === 'administrator' && <button className="topbar-settings-button" type="button" aria-label="Настройки" onClick={() => setWorkspace('settings')}><Settings size={17}/></button>}<button className="topbar-profile-button" type="button" onClick={() => setWorkspace('profile')}><span>{initials}</span><div><strong>{user.name || 'Администратор'}</strong><small>{user.role === 'administrator' ? 'Полный доступ' : user.role}</small></div></button></div></header>
      <div className="marketing-content"><Routes><Route path="/" element={<MarketingDashboardSummary/>}/><Route path="/chat" element={<CallCenterChatPage/>}/><Route path="/leads" element={<UnifiedLeadsPage/>}/><Route path="/calls" element={<Calls/>}/><Route path="/pipeline" element={<SalesFunnelPage/>}/><Route path="/advertising" element={<AdsManagerPage/>}/><Route path="/attribution" element={<AttributionPage/>}/><Route path="/analytics" element={<AnalyticsWorkspace/>}/><Route path="/integrations" element={<IntegrationManager/>}/><Route path="/journal" element={<JournalPage/>}/><Route path="/audit" element={<AuditPage/>}/><Route path="/sync-journal" element={<JournalPage/>}/><Route path="/architecture" element={<MarketingArchitecturePage/>}/><Route path="/legacy-leads" element={<LegacyLeadsPage/>}/></Routes></div>
