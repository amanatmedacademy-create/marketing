...
export default function V36Dashboard() {
  ...
  return <main className="v36-dashboard">
    <section className="v36-toolbar">
      ...
      <div className="v36-filter v36-filter--utm"><button className={utmSource||utmMedium||utmCampaign?'active':''} onClick={() => setUtmMenu(!utmMenu)}><SlidersHorizontal size={14}/>UTM фильтры{utmSource||utmMedium||utmCampaign?' •':''}<ChevronDown size={14}/></button>{utmMenu && <div className="v36-popover v36-popover--utm"><label><span>UTM Source</span><input value={utmSource} onChange={(event) => setUtmSource(event.target.value)} placeholder="Например: facebook"/></label><label><span>UTM Medium</span><input value={utmMedium} onChange={(event) => setUtmMedium(event.target.value)} placeholder="Например: cpc"/></label><label><span>UTM Campaign / ID</span><input value={utmCampaign} onChange={(event) => setUtmCampaign(event.target.value)} placeholder="Название или ID кампании"/></label></div>}</div>
    ...
    <section className="v36-kpis">{[
      ...
      ['Выручка',revenueMoney(totals.revenue),`Средний чек ${revenueMoney(totals.sales?totals.revenue/totals.sales:0)}`,'amber'],
      ['Расход',adMoney(totals.spend),`Валюта рекламного кабинета: USD`,'violet'],
    ...
    <section className="v36-summary">
      ...
      <article className="v36-card"><h2>Расход по платформам, USD</h2>{visiblePlatforms.map((platform,index) => <div className="v36-bar" key={platform.platform}><p><b>{platform.platform}</b><span>{adMoney(platform.spend)}</span></p><i><em style={{width:`${totals.spend?Math.max(2,platform.spend/totals.spend*100):0}%`,background:colors[index%colors.length]}}/></i></div>)}</article>
    ...
    <section className="v36-card v36-table-card"><header><div><h2>Детализация по кампаниям</h2><p>{filtered.length} кампаний · рекламные расходы в USD</p></div><div className="v36-table-actions"><button type="button" onClick={() => setAllOpen(true)}>Развернуть все</button><button type="button" onClick={() => setAllOpen(false)}>Свернуть все</button><div className="v36-mode"><button className={mode==='ads'?'active':''} onClick={() => setMode('ads')}>Реклама</button><button className={mode==='crm'?'active':''} onClick={() => setMode('crm')}>CRM-воронка</button></div></div></header><div className="v36-table"><table><thead>{mode==='ads'?<tr><th>Источник / кампания</th><th>UTM source</th><th>UTM medium</th><th>UTM campaign</th><th>ROAS</th><th>Расход, USD</th><th>Выручка, KZT</th><th>Показы</th><th>Охват</th><th>Клики</th><th>CTR</th><th>CPM, USD</th><th>Частота</th><th>Рекомендация</th></tr>:<tr><th>Источник / кампания</th><th>CPL, USD</th><th>Лиды рекламы</th><th>Лиды CRM</th><th>Целевые</th><th>В работе</th><th>Брак</th><th>Записаны</th><th>Пришли</th><th>Купили</th><th>Конверсия</th></tr>}</thead><tbody>{grouped.map((group) => <>{<tr className="v36-group" key={group.p.platform} onClick={() => setOpen((previous) => ({...previous,[group.p.platform]:!previous[group.p.platform]}))}><td><ChevronRight className={open[group.p.platform]?'open':''} size={14}/><b>{group.p.platform}</b><small>{group.rows.length} кампаний</small></td>{mode==='ads'?<><td/><td/><td/><td>{group.p.roas.toFixed(1)}x</td><td>{adMoney(group.p.spend)}</td><td>{revenueMoney(group.p.revenue)}</td><td>{num(group.p.impressions)}</td><td/><td/><td/><td/><td/><td/></>:<><td>{adMoney(group.p.leads?group.p.spend/group.p.leads:0)}</td><td/><td>{num(group.p.leads)}</td><td>{num(group.p.target_leads)}</td><td/><td/><td/><td>{num(group.p.arrived)}</td><td>{num(group.p.sales)}</td><td>{pct(group.p.sales,group.p.leads)}</td></>}</tr>}{open[group.p.platform]&&group.rows.map((row) => <tr key={row.key}><td><b>{row.campaign_name||row.source}</b><small>{row.campaign_id||row.source}</small></td>{mode==='ads'?<><td>{row.utm_source||'—'}</td><td>{row.utm_medium||'—'}</td><td>{row.utm_campaign||'—'}</td><td>{row.roas.toFixed(1)}x</td><td>{adMoney(row.spend)}</td><td>{revenueMoney(row.revenue)}</td><td>{num(row.impressions)}</td><td>{num(row.reach)}</td><td>{num(row.clicks)}</td><td>{row.ctr.toFixed(1)}%</td><td>{adMoney(row.cpm)}</td><td>{row.frequency.toFixed(1)}</td><td>{row.recommendation||'Наблюдать'}</td></>:<><td>{adMoney(row.cpl)}</td><td>{num(row.ads_leads)}</td><td>{num(row.crm_leads)}</td><td>{num(row.target_leads)}</td><td>{num(row.in_work)}</td><td>{num(row.rejected)}</td><td>{num(row.appointments)}</td><td>{num(row.arrived)}</td><td>{num(row.sales)}</td><td>{pct(row.sales,row.crm_leads)}</td></>}</tr>)}</>)}</tbody></table></div></section>
