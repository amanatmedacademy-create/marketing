...
import { CartesianGrid, Cell, ComposedChart, Line, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import { ChevronDown, ChevronRight, Eye, Filter, RefreshCw, SlidersHorizontal, X } from 'lucide-react';
...
export default function V36Dashboard() {
  ...
  return <main className="v36-dashboard">
    ...
    {!activeFilters && <section className="v36-card"><header><div><h2>Динамика лидов и продаж</h2><p>CRM-динамика по дням. Расходы отображаются отдельно только после корректной конвертации в KZT.</p></div></header><div className="v36-chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={chart}><CartesianGrid stroke="#1e2d4a" vertical={false}/><XAxis dataKey="label" stroke="#64748b"/><YAxis yAxisId="l" stroke="#64748b"/><Tooltip contentStyle={{background:'#060c1c',border:'1px solid #1e2d4a'}}/><Line yAxisId="l" dataKey="leads" name="Лиды" stroke="#2563eb" strokeWidth={3}/><Line yAxisId="l" dataKey="sales" name="Продажи" stroke="#22c55e" strokeWidth={3}/></ComposedChart></ResponsiveContainer></div></section>}
