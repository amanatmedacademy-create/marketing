...
function LoginPanel({ onAuthenticated, error, setError }: { onAuthenticated: (user: AppUser) => void; error: string | null; setError: (value: string | null) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true); setError(null);
    try {
      if (mode === 'login') await signInWithPassword(normalizedEmail, password, remember);
      else await registerNativeAccount({
  ...
  return <section className="auth-login-panel">
    <form className="auth-login-card" onSubmit={submit}>
      ...
      <button className="auth-submit" type="submit" disabled={busy}>{busy ? <LoaderCircle className="spin" size={19}/> : mode === 'login' ? <LockKeyhole size={18}/> : <UserRound size={18}/>}<span>{busy ? 'Проверяем данные…' : mode === 'login' ? 'Войти в систему' : 'Создать аккаунт'}</span></button>
...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    currentSession().then(async (session) => {
      ...
      if (active) { setUser(appUser); setError(null); }
  ...
  const switchCompany = async (companyId: string) => {
    ...
    const previous = activeCompanyId(); setSwitching(true); setError(null);
    try { setActiveCompanyId(companyId); await loadAppUser(); window.location.reload(); }
    catch (reason) { setActiveCompanyId(previous || null); setError(reason instanceof Error ? reason.message : String(reason)); setSwitching(false); }
  };
  ...
  if (!user || !context) return <div className="auth-screen">
    ...
    <div className="auth-layout">
      <section className="auth-showcase">
        ...
        <div className="auth-preview" aria-hidden="true"><header><span>Единое рабочее пространство</span><i>Secure</i></header><div className="auth-preview-kpis"><div><small>Организация</small><b>Tenant</b><em>Изолировано</em></div><div><small>Пользователи</small><b>Roles</b><em>Контроль</em></div><div><small>Доступ</small><b>SSO</b><em>Google + пароль</em></div></div><div className="auth-preview-chart">{[38,52,45,70,49,64,58,82,68,88,76,94].map((height,index)=><i key={index} style={{height:`${height}%`}}/>)}</div></div>
      </section>
