...
const columnOptions = [['status','Статус'],['result','Результат'],['cost','Цена результата'],['spend','Расход'],['impressions','Показы'],['reach','Охват*'],['frequency','Частота*'],['clicks','Клики'],['ctr','CTR'],['link_ctr','CTR ссылки'],['cpc','CPC'],['cpm','CPM'],['target','Целевые лиды'],['arrived','Пришли'],['sales','Продажи'],['revenue','Выручка платформы'],['utm','UTM'],['ids','Идентификаторы']] as const;
...
const formatNativeMoney=(value:number,currency?:string|null)=>{
  if(!currency)return `${new Intl.NumberFormat('ru-RU',{maximumFractionDigits:2}).format(value||0)} · валюта не определена`;
  try{return new Intl.NumberFormat('ru-RU',{style:'currency',currency,minimumFractionDigits:0,maximumFractionDigits:2}).format(value||0);}
  catch{return `${new Intl.NumberFormat('ru-RU',{maximumFractionDigits:2}).format(value||0)} ${currency}`;}
};
const isMetaRow = (row:Row) => {
  const provider = `${row.platform || ''} ${row.source || ''}`.toLowerCase();
  return provider.includes('meta') || provider.includes('facebook') || provider.includes('instagram');
};
...
export default function AdsManagerPage() {
  ...
  const load = async (range?:{from?:string;to?:string}) => {
    ...
    try {
      ...
      setData(JSON.parse(body) as ResponseData); setCurrencies(currencyResponse.accounts); setSelected([]);
  ...
  const currencyFor=(row:Row)=>currencyByAccount.get(accountCurrencyKey(row.platform,row.account_id))||null;
  ...
  const exportCsv=()=>{const headers=['Название','ID','Статус','Валюта кабинета','Результат','Цена результата','Расход','Показы','Охват приблизительный','Частота приблизительная','Клики','CTR','CTR ссылки','CPC','CPM','Целевые','Пришли','Продажи','Выручка платформы','UTM Source','UTM Medium','UTM Campaign','UTM Content'];const body=grouped.map(row=>[label(row),id(row),row.status,currencyFor(row)||'',row.leads,row.cost_per_result,row.spend,row.impressions,row.reach,row.frequency,row.clicks,row.ctr,row.link_ctr,row.cpc,row.cpm,row.target_leads,row.arrived,row.sales,row.revenue,row.utm_source,row.utm_medium,row.utm_campaign,row.utm_content].map(value=>`"${String(value).replace(/"/g,'""')}"`).join(';')).join('\n');const blob=new Blob([`\uFEFF${headers.join(';')}\n${body}`],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`imds-ads-${level}-${data.period.from}-${data.period.to}.csv`;link.click();URL.revokeObjectURL(url);};

  return <div className="ads-manager-page">
    <div className="heading"><span>Advertising workspace</span><h1>Рекламные кабинеты</h1><p>Расходы, unit economics и выручка рекламной платформы показываются в нативной валюте каждого кабинета. CRM-выручка и ROAS по CRM рассчитываются во вкладке «Аналитика».</p></div>
