...
type SavedView = {
  id:string; name:string; level:Level; account:string; status:string; campaign:string; adset:string; query:string; sort:SortKey; descending:boolean; columns:string[];
};
...
const columnOptions = [
  ['status','Статус'],['result','Результат'],['cost','Цена результата'],['spend','Расход'],['impressions','Показы'],['reach','Охват'],['frequency','Частота'],['clicks','Клики'],['ctr','CTR'],['link_ctr','CTR ссылки'],['cpc','CPC'],['cpm','CPM'],['target','Целевые лиды'],['arrived','Пришли'],['sales','Продажи'],['revenue','Выручка'],['utm','UTM'],['ids','Идентификаторы'],
] as const;
...
export default function AdsManagerPage() {
  ...
  const grouped = useMemo(() => {
    ...
    const filtered=needle?rows.filter(row=>[row.campaign_name,row.adset_name,row.ad_name,row.campaign_id,row.adset_id,row.ad_id,row.utm_source,row.utm_medium,row.utm_campaign,row.utm_content].some(value=>String(value||'').toLowerCase().includes(needle))):rows;
  ...
  const toggleAll = () => setSelected(allSelected ? selected.filter(key=>!grouped.some(row=>row.key===key)) : Array.from(new Set([...selected,...grouped.map(row=>row.key)])));
  ...
  const clearFilters=()=>{setAccount('all');setStatus('all');setCampaign('all');setAdset('all');setQuery('');setFrom('');setTo('');};

  const saveView=()=>{
    const name=viewName.trim(); if(!name) return;
    const view:SavedView={id:crypto.randomUUID(),name,level,account,status,campaign,adset,query,sort,descending,columns};
    const next=[...views,view]; setViews(next); localStorage.setItem('imds-ads-views',JSON.stringify(next)); setViewName('');
  };
  const applyView=(view:SavedView)=>{setLevel(view.level);setAccount(view.account);setStatus(view.status);setCampaign(view.campaign);setAdset(view.adset);setQuery(view.query);setSort(view.sort);setDescending(view.descending);setColumns(view.columns);};
  ...
  const exportCsv=()=>{const headers=['Название','ID','Статус','Результат','Цена результата','Расход USD','Показы','Охват','Частота','Клики','CTR','CTR ссылки','CPC USD','CPM USD','Целевые','Пришли','Продажи','Выручка KZT','UTM Source','UTM Medium','UTM Campaign','UTM Content'];const body=grouped.map(row=>[label(row),id(row),row.status,row.leads,row.cost_per_result,row.spend,row.impressions,row.reach,row.frequency,row.clicks,row.ctr,row.link_ctr,row.cpc,row.cpm,row.target_leads,row.arrived,row.sales,row.revenue,row.utm_source,row.utm_medium,row.utm_campaign,row.utm_content].map(value=>`"${String(value).replace(/"/g,'""')}"`).join(';')).join('\n');const blob=new Blob([`\uFEFF${headers.join(';')}\n${body}`],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`imds-ads-${level}.csv`;link.click();URL.revokeObjectURL(url);};

  return <div className="ads-manager-page">
    <div className="heading"><span>Meta Ads workspace</span><h1>Рекламные кабинеты</h1><p>Кампании, группы объявлений и объявления с фильтрами, сортировками и сохранёнными представлениями.</p></div>

    <section className="ads-saved-views">
      <strong>Представления</strong>
      <button type="button" className="active">Все объявления</button>
      {views.map(view=><div key={view.id}><button type="button" onClick={()=>applyView(view)}>{view.name}</button><button type="button" aria-label="Удалить представление" onClick={()=>deleteView(view.id)}><Trash2 size={13}/></button></div>)}
      <label><input value={viewName} onChange={event=>setViewName(event.target.value)} placeholder="Название представления"/><button type="button" onClick={saveView}><Save size={14}/>Сохранить</button></label>
    </section>

    <section className="ads-toolbar">
      <select value={account} onChange={event=>{setAccount(event.target.value);setCampaign('all');setAdset('all');}}><option value="all">Все рекламные аккаунты</option>{data.accounts.map(item=><option key={item.id} value={item.id}>{item.name} · {item.id}</option>)}</select>
      ...
      <button className={showFilters?'active':''} type="button" onClick={()=>setShowFilters(!showFilters)}><Filter size={15}/>Фильтры{activeFilters?` (${activeFilters})`:''}</button>
    </section>

    {showFilters&&<section className="ads-filter-panel">
      <label><span>Статус</span><select value={status} onChange={event=>setStatus(event.target.value)}><option value="all">Все статусы</option><option value="ACTIVE">Активные</option><option value="PAUSED">Остановленные</option><option value="UNKNOWN">Неизвестно</option></select></label>
      <label><span>Кампания</span><select value={campaign} onChange={event=>{setCampaign(event.target.value);setAdset('all');}}><option value="all">Все кампании</option>{campaigns.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label><span>Группа объявлений</span><select value={adset} onChange={event=>setAdset(event.target.value)}><option value="all">Все группы</option>{adsets.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label><span>Дата от</span><input type="date" value={from} onChange={event=>setFrom(event.target.value)}/></label>
      <label><span>Дата до</span><input type="date" value={to} onChange={event=>setTo(event.target.value)}/></label>
      <button type="button" onClick={load}>Применить даты</button>
      <button type="button" onClick={clearFilters}><X size={14}/>Сбросить</button>
    </section>}

    <section className="ads-controls">
      <label><SlidersHorizontal size={15}/><select value={sort} onChange={event=>setSort(event.target.value as SortKey)}><option value="spend">Расход</option><option value="impressions">Показы</option><option value="reach">Охват</option><option value="clicks">Клики</option><option value="ctr">CTR</option><option value="cpm">CPM</option><option value="leads">Результат</option><option value="cost_per_result">Цена результата</option><option value="sales">Продажи</option><option value="revenue">Выручка</option><option value="name">Название</option></select></label>
      <button onClick={()=>setDescending(!descending)}>{descending?'По убыванию':'По возрастанию'}</button>
      <div className="ads-columns"><button onClick={()=>setShowColumns(!showColumns)}><Columns3 size={15}/>Столбцы ({columns.length})<ChevronDown size={14}/></button>{showColumns&&<div>{columnOptions.map(([key,title])=><label key={key}><input type="checkbox" checked={columns.includes(key)} onChange={()=>toggle(key)}/>{title}</label>)}</div>}</div>
      <button onClick={exportCsv}><Download size={15}/>Экспорт</button>
      {selected.length>0&&<strong className="ads-selected">Выбрано: {selected.length}</strong>}
