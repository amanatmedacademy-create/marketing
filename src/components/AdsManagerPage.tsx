...
const emptyData:ResponseData={period:{from:'',to:'',days:30},accounts:[],rows:[]};

export default function AdsManagerPage() {
  const [data,setData] = useState<ResponseData>(emptyData);
  ...
  const load = async (range?:{from?:string;to?:string}) => {
    ...
    try {
      ...
      const response=await fetch(`/api/analytics/ad-manager?${params}`,{signal:controller.signal,cache:'no-store',headers:{Accept:'application/json'}});
      const body=await response.text();
      if(!response.ok) throw new Error(body||`HTTP ${response.status}`);
      const parsed=JSON.parse(body) as Partial<ResponseData>;
      if(!parsed.period||!Array.isArray(parsed.accounts)||!Array.isArray(parsed.rows)) throw new Error('Сервер вернул некорректный формат рекламных данных');
      setData(parsed as ResponseData); setSelected([]);
    } catch(reason){
      if(reason instanceof DOMException&&reason.name==='AbortError') setError('Сервер рекламных данных не ответил за 20 секунд. Нажмите «Повторить» или проверьте подключение Meta/TikTok.');
      else setError(reason instanceof Error?reason.message:String(reason));
      setData(previous=>previous.rows.length?previous:emptyData);
    } finally {
