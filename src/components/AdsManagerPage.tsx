...
export default function AdsManagerPage() {
  ...
  const exportCsv=()=>{const headers=['Название','ID','Статус','Валюта кабинета','Результат','Цена результата','Расход','Показы','Охват приблизительный','Частота приблизительная','Клики','CTR','CTR ссылки','CPC','CPM','Целевые','Пришли','Продажи','Выручка KZT','UTM Source','UTM Medium','UTM Campaign','UTM Content'];const body=grouped.map(row=>[label(row),id(row),row.status,currencyFor(row)||'',row.leads,row.cost_per_result,row.spend,row.impressions,row.reach,row.frequency,row.clicks,row.ctr,row.link_ctr,row.cpc,row.cpm,row.target_leads,row.arrived,row.sales,row.revenue,row.utm_source,row.utm_medium,row.utm_campaign,row.utm_content].map(value=>`"${String(value).replace(/"/g,'""')}"`).join(';')).join('\n');const blob=new Blob([`\uFEFF${headers.join(';')}\n${body}`],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`imds-ads-${level}-${data.period.from}-${data.period.to}.csv`;link.click();URL.revokeObjectURL(url);};

  return <div className="ads-manager-page">
    <div className="heading"><span>Advertising workspace</span><h1>Рекламные кабинеты</h1><p>Расходы и unit economics показываются в нативной валюте каждого рекламного кабинета. CRM-выручка — в KZT.</p></div>
