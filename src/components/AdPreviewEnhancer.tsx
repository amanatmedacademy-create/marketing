...
function isAdRow(row: HTMLElement): boolean {
  ...
  if (row.closest('.ads-table-wrap')) {
    const text = row.innerText || '';
    return /(?:Объявление|Ad)\s*[·:]/i.test(text) || Boolean(row.querySelector('strong'));
  }
  return false;
}
  ...
  return <div className="ad-preview-layer" role="dialog" aria-modal="true" aria-label="Предпросмотр объявления">
    ...
    <aside className="ad-preview-drawer">
      ...
      <section className={`ad-preview-stage mode-${mode}`}>
        ...
        {error && <div className="ad-preview-state error">{error}</div>}
        {!loading && !error && preview?.previewHtml && <iframe title="Meta ad preview" sandbox="allow-scripts allow-same-origin allow-popups" srcDoc={preview.previewHtml}/>} 
      ...
      {content && <footer className="ad-preview-meta"><span>ID объявления: {content.adId}</span>{content.destinationUrl && <a href={content.destinationUrl} target="_blank" rel="noreferrer">Открыть ссылку <ExternalLink size={13}/></a>}</footer>}
    </aside>
  </div>;
}
