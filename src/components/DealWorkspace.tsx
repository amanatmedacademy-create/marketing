...
export default function DealWorkspaceHost() {
  ...
  return <div className="deal-workspace-layer" role="dialog" aria-modal="true" aria-label={`Сделка ${deal.fullName}`}>
    ...
    <section className="deal-workspace-panel">
      <header className="deal-workspace-header">
        ...
        <div className="deal-workspace-header-actions">
          {deal.phone && <a href={phoneHref(deal.phone)} title="Позвонить"><Phone/></a>}
          {deal.phone && <a href={whatsappHref(deal.phone)} target="_blank" rel="noreferrer" title="Открыть WhatsApp"><MessageCircle/></a>}
        ...
        <main className="deal-workspace-feed">
          ...
          {tab !== 'links' && <form className="deal-workspace-composer" onSubmit={(event) => void postActivity(event)}>
            <div className="composer-modes">
              ...
              {deal.phone && <a href={phoneHref(deal.phone)}><PhoneCall/> Звонок</a>}
              {deal.phone && <a href={whatsappHref(deal.phone)} target="_blank" rel="noreferrer"><Send/> Сообщение</a>}
            </div>
