...
const supported: ProviderDefinition[] = [
  ...
  { provider: 'tiktok', title: 'TikTok Ads', description: 'Кампании, расходы, объявления и лиды TikTok.', fields: [
  ...
  { provider: 'bitrix', title: 'Bitrix24', description: 'Лиды, сделки, стадии, визиты и продажи из CRM.', fields: [
  ...
  { provider: 'n8n', title: 'n8n', description: 'Webhooks, сценарии и резервный импорт данных.', fields: [
...
async function withTimeout<T>(promise: Promise<T>, timeoutMs = 8000): Promise<T> {
  let timer = 0;
  try {
    return await Promise.race([promise, new Promise<T>((_, reject) => { timer = window.setTimeout(() => reject(new Error('Сервер интеграций не ответил вовремя')), timeoutMs); })]);
  } finally {
...
export default function IntegrationManager() {
  ...
  const runMap = useMemo(() => {
    ...
    for (const run of status?.runs || []) {
      if (['meta', 'tiktok', 'bitrix', 'n8n'].includes(run.source) && !map.has(run.source as IntegrationProvider)) map.set(run.source as IntegrationProvider, run);
    }
    return map;
  }, [status?.runs]);
  ...
  const updateField = (provider: IntegrationProvider, name: string, value: string) => setForms((previous) => ({ ...previous, [provider]: { ...previous[provider], [name]: value } }));
  const openEditor = (provider: IntegrationProvider) => { const definition = supported.find((item) => item.provider === provider); if (definition) setEditor(definition); };
  ...
  const save = async (definition: ProviderDefinition) => {
    ...
    if (missing.length) return setMessage({ type: 'error', text: `Заполните: ${missing.map((field) => field.label).join(', ')}` });
    if (definition.provider === 'meta' && !values.accessToken && !current?.secretFields.accessToken) return setMessage({ type: 'error', text: 'Для Meta используйте вход через Facebook или укажите резервный access token.' });
  ...
  if (!isAdmin) return <section className="iv2-denied"><AlertTriangle size={28}/><h1>Недостаточно прав</h1><p>Настройка интеграций доступна только администратору.</p></section>;

  return <div className="iv2-page">
    <header className="iv2-header"><div><span>INTEGRATIONS</span><h1>Интеграции</h1><p>Подключения рекламных кабинетов, CRM, автоматизации и коммуникаций.</p></div><button type="button" onClick={() => void load()} disabled={loading || Boolean(busy)}>{loading ? <LoaderCircle className="spin" size={16}/> : <RefreshCw size={16}/>} Обновить</button></header>
    <section className="iv2-summary"><article><CheckCircle2 size={22}/><div><span>Подключено</span><strong>{connectedCount} из {cards.length}</strong></div></article><article><Plug size={22}/><div><span>API</span><strong>{message?.type === 'error' ? 'Ошибка' : loading ? 'Проверка' : 'Работает'}</strong></div></article><article><Settings size={22}/><div><span>Архитектура</span><strong>React без DOM-мутаций</strong></div></article></section>
