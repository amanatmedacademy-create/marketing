...
import { marketingApi, type IntegrationProvider, type WabaConfigResponse } from '../services/api';
...
function addPremiumModalScreen(modal: HTMLElement) {
  ...
  screen.innerHTML = `
    <div class="premium-instruction-heading"><div><h3>Как подключить ${modal.querySelector('header h2')?.textContent || 'интеграцию'}</h3><p>Выполните четыре шага. После подключения данные начнут синхронизироваться автоматически.</p></div></div>
    <div class="premium-instruction-steps">
      ${steps.map((step, index) => `<article class="premium-instruction-step"><div class="premium-step-title"><span>${index + 1}</span><strong>${step.title}</strong></div><p>${step.text}</p><div class="connection-guide-visual connection-guide-visual--${step.visual}" aria-hidden="true"><i></i><b></b><em></em></div><div class="premium-step-requirement"><strong>Что потребуется</strong>${step.requirement}</div></article>`).join('')}
    </div>
    <div class="premium-instruction-footer"><button type="button" class="premium-start-button">Начать подключение</button><button type="button" class="premium-settings-button">Открыть настройки</button><span>Подключение выполняется через безопасную OAuth-авторизацию.</span></div>`;
...
async function loadFacebookSdk(config: WabaConfigResponse): Promise<FacebookSdk> {
  const current = (window as unknown as { FB?: FacebookSdk }).FB;
...
async function launchWabaSignup(button: HTMLButtonElement, status: HTMLElement) {
  ...
  try {
    ...
    const assetPromise = waitForEmbeddedSignupResult();
    ...
    const [code, assets] = await Promise.all([codePromise, assetPromise]);
    await marketingApi.connectWaba({ code, ...assets });
    status.textContent = `Подключено · WABA ${assets.wabaId}`;
...
function addWabaCard() {
  if (document.querySelector('[data-waba-card]')) return;
  const sections = Array.from(document.querySelectorAll<HTMLElement>('.integration-catalog-section'));
  const advertising = sections.find((section) => /рекламные кабинеты/i.test(section.querySelector('h2')?.textContent || ''));
  const grid = advertising?.querySelector<HTMLElement>('.integration-catalog-grid');
  if (!grid) return;

  const card = document.createElement('article');
  card.dataset.wabaCard = 'true';
  card.className = 'integration-catalog-card integration-premium-card integration-tone-meta integration-state-disconnected';
  card.innerHTML = `
    <div class="integration-card-top"><span class="integration-card-logo">WA</span><em data-waba-state>Проверяем</em></div>
    <strong>WhatsApp Business</strong>
    <p>Подключение WhatsApp Business Account и номера через Meta Embedded Signup.</p>
    <div class="integration-card-tags"><span>Cloud API</span><span>WABA</span><span>Сообщения</span></div>
    <div class="integration-premium-metrics"><div><span>WABA ID</span><strong data-waba-id>—</strong></div><div><span>Phone Number ID</span><strong data-phone-id>—</strong></div><div><span>Статус</span><strong data-waba-status>—</strong></div></div>
    <div class="integration-premium-actions"><button type="button" class="integration-premium-primary" data-waba-connect>Подключить через Facebook</button></div>`;
  grid.appendChild(card);

  const button = card.querySelector<HTMLButtonElement>('[data-waba-connect]')!;
  const state = card.querySelector<HTMLElement>('[data-waba-state]')!;
  const status = card.querySelector<HTMLElement>('[data-waba-status]')!;
  button.addEventListener('click', () => void launchWabaSignup(button, status));

  void marketingApi.wabaConfig().then((config) => {
    state.textContent = config.connected ? 'Подключено' : config.configured ? 'Готово к подключению' : 'Нужна настройка';
    card.classList.toggle('integration-state-connected', config.connected);
    card.classList.toggle('integration-state-disconnected', !config.connected);
    const values = config.connection?.values;
    card.querySelector<HTMLElement>('[data-waba-id]')!.textContent = values?.wabaId || '—';
    card.querySelector<HTMLElement>('[data-phone-id]')!.textContent = values?.phoneNumberId || '—';
    status.textContent = config.connected ? 'Активно' : config.configured ? 'Можно подключать' : (config.error || 'Не настроено');
    if (config.connected) button.textContent = 'Переподключить';
  }).catch((error) => {
