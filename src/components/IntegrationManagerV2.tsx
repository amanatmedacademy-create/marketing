...
import { Cable, CheckCircle2, LoaderCircle, Play, Save, Trash2, XCircle } from 'lucide-react';
import type {
...
export default function IntegrationManagerV2() {
  ...
  const load = async () => {
    ...
    try {
      const [nextStatus, nextConfigs] = await Promise.all([
        ...
        api<IntegrationConfigResponse>('/integrations/config'),
      ]);
      setStatus(nextStatus);
      setConfigs(nextConfigs);
      setForms((previous) => {
        const next = { ...previous } as FormState;
        for (const config of nextConfigs.providers) next[config.provider] = { ...previous[config.provider], ...config.values };
        return next;
      });
      setMessage(null);
    } catch (error) {
  ...
  const saveAndTest = async (definition: ProviderDefinition) => {
    ...
    try {
      ...
      await api(`/integrations/config/${definition.provider}`, { method: 'PUT', body: JSON.stringify(payload) });
      await api(`/integrations/test/${definition.provider}`, { method: 'POST', body: '{}' });
      await load();
      setForms((previous) => ({
        ...previous,
        [definition.provider]: {
  ...
  const disconnect = async (provider: IntegrationProvider) => {
    ...
    setMessage(null);
    try {
      await api(`/integrations/config/${provider}`, { method: 'DELETE' });
  ...
  const sync = async (source: IntegrationProvider | 'all', days: number) => {
    ...
    setMessage(null);
    try {
      await api('/integrations/sync', { method: 'POST', body: JSON.stringify({ source, days }) });
      await load();
  ...
  if (loading) return <section className="panel integration-unlock"><LoaderCircle className="spin" size={24}/><div><h2>Загрузка интеграций</h2><p className="note">Проверяем подключения и права администратора.</p></div></section>;

  return <div className="stack">
    <div className="integration-heading">
      <div className="heading"><span>Data connections</span><h1>Подключение интеграций</h1><p>Доступ открыт по роли администратора. Реквизиты шифруются в Cloudflare Worker и не возвращаются в браузер.</p></div>
      <button className="button button--primary" onClick={() => void sync('all', 90)} disabled={Boolean(busy)}><Play size={16}/>{busy === 'sync:all' ? 'Синхронизация…' : 'Загрузить 90 дней'}</button>
    </div>
