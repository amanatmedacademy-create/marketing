...
function dealStatus(deal: FunnelDeal): { key: string; label: string } {
  if (deal.paid) return { key: 'paid', label: 'Оплачено' };
  if (deal.status === 'won') return { key: 'won', label: 'Успешно' };
  if (deal.status === 'lost') return { key: 'lost', label: 'Потеряно' };
  return { key: 'open', label: 'В работе' };
}
...
export function SalesFunnelKanban({ pipelines, selectedPipelineId, deals, users, draggingId, onSelectPipeline, onDraggingChange, onMove, onCreatePipeline, onManagePipeline }: Props) {
  ...
  return <section className="funnel-v2-kanban-shell">
    ...
    <div className="funnel-v2-stage-strip">
      {pipeline.stages.map((stage, index) => <span key={stage.id}><b className={`stage-tone stage-tone--${stage.stageType}`} style={{ '--stage-user-color': stage.color } as CSSProperties}>{stage.name}</b>{index < pipeline.stages.length - 1 && <ArrowRight size={13}/>}</span>)}
    </div>
    ...
    <div className="funnel-v2-kanban-scroll">
      <div className="funnel-v2-board" style={{ gridTemplateColumns: `repeat(${Math.max(1, pipeline.stages.length)}, minmax(230px, 1fr))` }}>
        {pipeline.stages.map((stage) => {
          ...
          return <article className={`funnel-v2-column stage-type-${stage.stageType} ${draggingId ? 'drag-active' : ''}`} key={stage.id} onDragOver={(event) => event.preventDefault()} onDrop={() => void drop(stage.id)}>
            <header style={{ '--stage-user-color': stage.color } as CSSProperties}>
            ...
            <div className="funnel-v2-column-body">
              {stageDeals.map((deal) => {
                ...
                return <section className={`funnel-v2-deal priority-${deal.priority.toLowerCase()} status-${status.key}`} key={deal.id} draggable
                  ...
                  <header><span>{initials(deal.fullName)}</span><div><strong>{deal.fullName}</strong><small>{deal.phone || deal.email || 'Контакт не указан'}</small></div><i className={`priority-chip priority-chip--${deal.priority.toLowerCase()}`}>{PRIORITY[deal.priority]}</i></header>
                  ...
                  <div className="funnel-v2-deal-meta">
                    <SourceBadge source={deal.source}/>
                    <span className={`deal-status-chip deal-status-chip--${status.key}`}>{status.label}</span>
                    <span className="deal-stage-age"><Clock3 size={11}/>{age(deal.stageEnteredAt)}</span>
                  </div>
                  {deal.amount > 0 && <div className={`funnel-v2-deal-amount ${deal.paid ? 'is-paid' : ''}`}><CircleDollarSign size={14}/><span>{deal.paid ? 'Оплачено' : 'Сумма сделки'}</span><strong>{money.format(deal.amount)}</strong></div>}
