...
export function SalesFunnelKanban({
  leads,
  users,
  draggingId,
  onDraggingChange,
  ...
  const loadBoards = useCallback(async () => {
    ...
    try {
      ...
      setBoardError(error instanceof Error ? error.message : 'Не удалось загрузить канбаны');
    } finally {
  ...
  const openEdit = () => {
    ...
    setDraft(blankDraft(selectedBoard));
    setModalOpen(true);
    setBoardError('');
  };
  ...
  const duplicateBoard = () => {
    setEditingId(null);
    setDraft({ ...blankDraft(selectedBoard), name: `${selectedBoard.name} — копия`, isDefault: false, sortOrder: (selectedBoard.sortOrder || 0) + 10 });
    setModalOpen(true);
    setBoardError('');
  };
  ...
  const saveBoard = async (event: FormEvent) => {
    ...
    if (!draft.name.trim()) { setBoardError('Укажите название канбана'); return; }
    if (!draft.columns.some((column) => column.visible)) { setBoardError('Оставьте хотя бы одну видимую колонку'); return; }
    setSaving(true);
    setBoardError('');
    try {
      ...
      setBoardError(error instanceof Error ? error.message : 'Не удалось сохранить канбан');
    } finally {
      setSaving(false);
    }
  };
  ...
  const archiveBoard = async () => {
    if (!editingId || boards.length < 2 || !window.confirm('Архивировать этот канбан? Лиды останутся в системе.')) return;
    setSaving(true);
    setBoardError('');
    try {
      ...
      setBoardError(error instanceof Error ? error.message : 'Не удалось архивировать канбан');
    } finally {
      setSaving(false);
    }
  };
  ...
  const dropLead = async (column: FunnelKanbanColumn) => {
    ...
    if (lead.stage !== column.stage && column.wipLimit > 0 && targetCount >= column.wipLimit) {
      ...
      onError(`Колонка «${column.title}» заполнена: лимит ${column.wipLimit}`);
      return;
    }
  ...
  return <section className="rop-kanban-shell" aria-label="Канбан воронки продаж">
    <header className="rop-kanban-toolbar">
      <div className="rop-board-tabs" role="tablist" aria-label="Канбан-доски">
        ...
        >
          ...
          {board.isDefault && <i title="Основной канбан">●</i>}
        </button>)}
      ...
      <div className="rop-board-actions">
        ...
        <button type="button" onClick={openCreate}>+ Канбан</button>
    ...
    <div className="rop-board-caption">
      ...
      <div><b>{boardLeads.length}</b><span>лидов</span><b>{visibleColumns.length}</b><span>колонок</span></div>
    </div>
    ...
    <div className="rop-kanban-scroll" tabIndex={0} aria-label="Горизонтальная прокрутка канбана">
      <div className="rop-config-board">
        {visibleColumns.map((column) => {
          ...
          return <article
            className={`rop-column rop-config-column stage-${column.stage.toLowerCase()} ${draggingId ? 'drag-active' : ''} ${limitReached ? 'limit-reached' : ''}`}
            key={column.stage}
            onDragOver={(event) => event.preventDefault()}
            onDrop={() => void dropLead(column)}
          >
            <header style={{ borderTopColor: column.color }}>
              <div><span>{column.title}</span><small>{column.subtitle}</small></div>
              <b>{stageLeads.length}{column.wipLimit > 0 ? ` / ${column.wipLimit}` : ''}</b>
              {selectedBoard.showTotals && <em>{money.format(stageAmount)}</em>}
            </header>
            <div className="rop-column-body">
              {stageLeads.map((lead) => <section
                className={`rop-lead-card priority-${lead.priority.toLowerCase()}`}
                key={lead.id}
                draggable
                onDragStart={(event: DragEvent) => {
                  event.dataTransfer.effectAllowed = 'move';
                  event.dataTransfer.setData('text/plain', lead.id);
                  onDraggingChange(lead.id);
                }}
    ...
    {modalOpen && <div className="rop-board-modal-layer" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setModalOpen(false); }}>
      <form className="rop-board-modal" onSubmit={(event) => void saveBoard(event)}>
        <header><div><small>ВОРОНКА ПРОДАЖ</small><h2>{editingId ? 'Настроить канбан' : 'Новый канбан'}</h2></div><button type="button" onClick={() => setModalOpen(false)}>×</button></header>
        <div className="rop-board-modal-body">
          <section className="rop-board-settings-grid">
            ...
            <label><span>Порядок доски</span><input type="number" min="0" max="9999" value={draft.sortOrder || 0} onChange={(event) => setDraft({ ...draft, sortOrder: Number(event.target.value) })}/></label>
            ...
            <label className="rop-board-check"><input type="checkbox" checked={draft.showTotals} onChange={(event) => setDraft({ ...draft, showTotals: event.target.checked })}/><span>Показывать суммы в колонках</span></label>
            <label className="rop-board-check"><input type="checkbox" checked={draft.isDefault === true} onChange={(event) => setDraft({ ...draft, isDefault: event.target.checked })}/><span>Основной канбан</span></label>
          </section>

          <section className="rop-board-filter-settings">
            <header><div><strong>Фильтры доски</strong><span>Пустой выбор показывает все лиды</span></div></header>
          ...
          <section className="rop-board-column-settings">
            <header><div><strong>Колонки и этапы</strong><span>Можно скрывать, переименовывать, менять порядок, цвет и WIP-лимит</span></div></header>
            {draft.columns.map((column, index) => <article key={column.stage}>
              <label className="rop-column-visible"><input type="checkbox" checked={column.visible} onChange={(event) => updateColumn(index, { visible: event.target.checked })}/><span>{column.stage}</span></label>
              <input aria-label="Название колонки" value={column.title} onChange={(event) => updateColumn(index, { title: event.target.value })}/>
              <input aria-label="Подпись колонки" value={column.subtitle} onChange={(event) => updateColumn(index, { subtitle: event.target.value })}/>
              <input className="rop-column-color" aria-label="Цвет колонки" type="color" value={column.color} onChange={(event) => updateColumn(index, { color: event.target.value })}/>
              ...
              <div className="rop-column-order"><button type="button" disabled={index === 0} onClick={() => moveColumn(index, -1)}>↑</button><button type="button" disabled={index === draft.columns.length - 1} onClick={() => moveColumn(index, 1)}>↓</button></div>
            </article>)}
          </section>
        </div>
        {boardError && <div className="rop-board-modal-error">{boardError}</div>}
        <footer>
          <div>{editingId && boards.length > 1 && <button type="button" className="button rop-board-archive" disabled={saving} onClick={() => void archiveBoard()}>Архивировать</button>}</div>
          <div><button type="button" className="button button-secondary" disabled={saving} onClick={() => setModalOpen(false)}>Отмена</button><button className="button button-primary" disabled={saving}>{saving ? 'Сохранение…' : 'Сохранить канбан'}</button></div>
        </footer>
      </form>
    </div>}
  </section>;
