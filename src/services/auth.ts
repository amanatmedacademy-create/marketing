...
async function refreshSession(session: StoredSession): Promise<StoredSession | null> {
  ...
  if (!response.ok) return null; const payload = await response.json() as Record<string, unknown>; const accessToken = typeof payload.access_token === 'string' ? payload.access_token : ''; if (!accessToken) return null;
  const next: StoredSession = { access_token: accessToken, refresh_token: typeof payload.refresh_token === 'string' ? payload.refresh_token : session.refresh_token, token_type: typeof payload.token_type === 'string' ? payload.token_type : 'bearer', expires_at: Math.floor(Date.now() / 1000) + Number(payload.expires_in || 3600) };
  writeStoredSession(next); return next;
}
...
export async function signOutSession(): Promise<void> { writeStoredSession(null); await fetch('/api/auth/logout', { method: 'POST' }).catch(() => undefined); }
export async function authFetch(input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> { const session = await currentSession(); const headers = new Headers(init.headers || {}); if (session?.access_token) headers.set('authorization', `Bearer ${session.access_token}`); return fetch(input, { ...init, headers }); }
