export type BillingPlan = { code: string; name: string; description?: string | null; amount?: number | null; currency?: string | null; interval?: string | null; current?: boolean; recommended?: boolean; limits?: Record<string, number> };
export type BillingAddOn = { code: string; name: string; description?: string | null; amount?: number | null; currency?: string | null; unit?: string | null; quantity?: number; active?: boolean };
export type BillingInvoice = { id: string; number?: string | null; status?: string | null; amount?: number | null; currency?: string | null; issuedAt?: string | null; dueAt?: string | null; paidAt?: string | null; url?: string | null };
export type BillingMethod = { method: string; displayName: string; instructions: string | null; isDefault: boolean };
export type BillingState = { subscriptionStatus: string | null; trialEndsAt: string | null; periodEndsAt: string | null; graceEndsAt: string | null; accessEndsAt: string | null; renewalMode: string | null; currency: string; paymentMethods: BillingMethod[]; defaultPaymentMethod: string | null };
...
type LinkResponse = { url?: string; checkoutUrl?: string; portalUrl?: string; error?: string };
...
export async function startCheckout(input: { kind: 'subscription' | 'addon'; planCode?: string; addonCode?: string; quantity?: number }) {
  ...
  if (!url) throw new Error(result.error || 'CHECKOUT_URL_MISSING');
