...
export type BranchList = { items: Branch[]; restricted: boolean; canManage: boolean };
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  const response = await authFetch(path, { ...init, cache: 'no-store', headers: { Accept: 'application/json', ...(init.body ? { 'Content-Type': 'application/json' } : {}), ...(init.headers || {}) } });
...
export async function createBranch(input: { name: string; code?: string; city?: string; address?: string; phone?: string; timezone?: string }): Promise<void> {
  await request('/api/branches', { method: 'POST', body: JSON.stringify(input) });
}
export async function updateBranch(id: string, input: Partial<Pick<Branch, 'name' | 'code' | 'city' | 'address' | 'phone' | 'timezone' | 'status'>>): Promise<void> {
  await request(`/api/branches/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) });
}
