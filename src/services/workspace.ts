...
export const workspaceApi = {
  ...
  save: (block: WorkspaceBlockInput) => request<{ block: WorkspaceBlock }>('/api/workspace/blocks', {
    method: 'POST',
    body: JSON.stringify(block),
  }),
  patch: (id: string, patch: Partial<WorkspaceBlockInput>) => request<{ block: WorkspaceBlock }>(`/api/workspace/blocks/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(patch),
  }),
  remove: (id: string) => request<{ ok: true; id: string }>(`/api/workspace/blocks/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  }),
};
