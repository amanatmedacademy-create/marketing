...
export default function ClinicSchedulePage() {
  ...
  const saveDraft=async()=>{
    if(!draft) return;
    if(draft.kind==='block'&&!isAdmin){toast('Закрывать время может только администратор','warn');return;}
    setSaving(true);
    try {
  ...
  const deleteBlock=async(block:ScheduleBlock)=>{ if(!isAdmin){toast('Открывать закрытое время может только администратор','warn');return;} if(!window.confirm(`Открыть время «${block.title}»?`)) return; try { await api('/api/clinic-schedule',{method:'POST',body:JSON.stringify({action:'delete_block',id:block.id})}); toast('Время снова доступно'); await load(); } catch(error){toast(error instanceof Error?error.message:String(error),'err');} };
  ...
  const remove=async(item:Appointment)=>{ if(!isAdmin){toast('Удалять визит может только администратор','warn');return;} if(!window.confirm('Удалить запись без возможности восстановления?')) return; try { await api('/api/clinic-schedule',{method:'POST',body:JSON.stringify({action:'delete_appointment',id:item.id})}); setSelected(null); toast('Запись удалена'); await load(); } catch(error){toast(error instanceof Error?error.message:String(error),'err');} };
  ...
  return <div className="mis-schedule-page">
    <header className="mis-schedule-topbar">
      ...
      <button className="sync" onClick={()=>void load()} disabled={loading} title="Синхронизировать"><RefreshCw size={16}/></button>
    </header>
    ...
    <section className="mis-schedule-mobile" aria-label="Мобильное расписание">
      <header><div><CalendarDays size={17}/><strong>{fullDate(selectedDate)}</strong></div><button onClick={()=>void load()} disabled={loading}><RefreshCw size={15}/></button></header>
      ...
      <button className="mobile-add" onClick={()=>createDraft(mobileDoctor||mobileDoctors[0]?.id||'',9*60)} disabled={!mobileDoctors.length}><Plus size={17}/> Новая запись</button>
    </section>
