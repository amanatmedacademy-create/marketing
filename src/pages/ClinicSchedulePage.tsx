...
export default function ClinicSchedulePage({ crmContext }: { crmContext?: ScheduleCrmContext }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const next=await api<Snapshot>(`/api/clinic-schedule?date=${selectedDate}`);
      setData({...next,blocks:next.blocks||[]});
      setSelected((current)=>current && !isExternalBusy(current) ? next.appointments.find((x)=>x.id===current.id && !isExternalBusy(x)) || null : null);
    } catch(error) {
  ...
  const counters=useMemo(()=>({
    today:new Set(marketingAppointments.map((x)=>x.patient_id||x.phone||x.patient_name)).size,
    arrived:marketingAppointments.filter((x)=>x.status==='ARRIVED'||x.status==='COMPLETED').length,
    completed:marketingAppointments.filter((x)=>x.status==='COMPLETED').length,
    cancelled:marketingAppointments.filter((x)=>x.status==='CANCELLED'||x.status==='NO_SHOW').length,
  }),[marketingAppointments]);
  ...
  const saveDraft=async()=>{
    ...
    setSaving(true);
    try {
      const startsAt=appointmentIso(draft.date,draft.time);
      const endsAt=new Date(new Date(startsAt).getTime()+draft.duration*60000).toISOString();
      if(draft.kind==='block') {
  ...
  const changeStatus=async(item:Appointment,status:AppointmentStatus)=>{ if(isExternalBusy(item)) return; try { await api('/api/clinic-schedule',{method:'POST',body:JSON.stringify({action:'set_appointment_status',id:item.id,status})}); toast(`Статус: ${STATUS_LABELS[status]}`); await load(); } catch(error){toast(error instanceof Error?error.message:String(error),'err');} };
  const remove=async(item:Appointment)=>{ if(isExternalBusy(item)) return; if(!window.confirm('Удалить запись без возможности восстановления?')) return; try { await api('/api/clinic-schedule',{method:'POST',body:JSON.stringify({action:'delete_appointment',id:item.id})}); setSelected(null); toast('Запись удалена'); await load(); } catch(error){toast(error instanceof Error?error.message:String(error),'err');} };
  const move=async(item:Appointment,doctorId:string,minute:number)=>{ if(isExternalBusy(item)) return; const startsAt=appointmentIso(selectedDate,minuteLabel(minute)); const endsAt=new Date(new Date(startsAt).getTime()+durationMinutes(item)*60000).toISOString(); try { await api('/api/clinic-schedule',{method:'POST',body:JSON.stringify({action:'move_appointment',id:item.id,doctor_id:doctorId,starts_at:startsAt,ends_at:endsAt})}); toast('Запись перенесена'); await load(); } catch(error){toast(error instanceof Error?error.message:String(error),'err');} };
      ...
      <main className="mis-schedule-stage">
        ...
        <div ref={scrollRef} className={`mis-schedule-scroll step-${step}`}>
          <div className="mis-schedule-inner" style={{minWidth:52+Math.max(activeDoctors.length,1)*224}}>
            ...
            <div className="grid-body" style={{height:timeSlots.length*SLOT_HEIGHT}}>
              <div className="time-column">{timeSlots.map((m)=><div className="time-slot" style={{height:SLOT_HEIGHT}} key={m}><span>{minuteLabel(m)}</span></div>)}</div>
              {activeDoctors.map((doctor)=><div className={`staff-column ${draggedId?'drag-active':''}`} key={doctor.id}>
                {timeSlots.map((m)=>{const slotStart=appointmentIso(selectedDate,minuteLabel(m));const slotEnd=new Date(new Date(slotStart).getTime()+step*60000).toISOString();const blocked=(dayBlocksByDoctor.get(doctor.id)||[]).some((block)=>overlaps(slotStart,slotEnd,block.starts_at,block.ends_at));return <button key={m} className={`column-slot ${blocked?'schedule-blocked':''}`} style={{height:SLOT_HEIGHT}} title={blocked?'Время закрыто':'Двойной клик — создать запись'} onDoubleClick={()=>{if(!blocked)createDraft(doctor.id,m)}} onDragOver={(e)=>{if(!blocked)e.preventDefault()}} onDrop={(e:DragEvent<HTMLButtonElement>)=>{if(blocked)return;e.preventDefault();const id=draggedId||e.dataTransfer.getData('text/plain');setDraggedId('');const item=data.appointments.find((x)=>x.id===id);if(item&&!isExternalBusy(item)) void move(item,doctor.id,m);}}/>})}
                {(dayBlocksByDoctor.get(doctor.id)||[]).map((block)=>{const top=((minutes(block.starts_at)-WORKDAY_START)/step)*SLOT_HEIGHT;const height=Math.max(42,(durationMinutes(block)/step)*SLOT_HEIGHT-6);return <article key={block.id} className={`schedule-block block-${block.block_type}`} style={{top,height}} onClick={()=>void deleteBlock(block)} title="Нажмите, чтобы открыть время"><div><LockKeyhole size={12}/><strong>{BLOCK_LABELS[block.block_type]}</strong></div><span>{timeLabel(block.starts_at)}–{timeLabel(block.ends_at)}</span><small>{block.title}</small></article>})}
                {(dayAppointmentsByDoctor.get(doctor.id)||[]).map((item)=>{const top=((minutes(item.starts_at)-WORKDAY_START)/step)*SLOT_HEIGHT;const height=Math.max(42,(durationMinutes(item)/step)*SLOT_HEIGHT-6);const externalBusy=isExternalBusy(item);const movable=!externalBusy&&(item.status==='BOOKED'||item.status==='CONFIRMED');return <article draggable={movable} key={item.id} onDragStart={(e)=>{if(!movable){e.preventDefault();return;}setDraggedId(item.id);e.dataTransfer.setData('text/plain',item.id)}} onDragEnd={()=>setDraggedId('')} onMouseEnter={externalBusy?undefined:(e)=>{const rect=e.currentTarget.getBoundingClientRect();setTooltip({appointment:item,x:Math.min(window.innerWidth-340,rect.right+8),y:Math.max(12,Math.min(window.innerHeight-250,rect.top))});}} onMouseLeave={externalBusy?undefined:()=>setTooltip(null)} onClick={externalBusy?undefined:()=>{setTooltip(null);setSelected(item)}} className={`appointment status-${item.status.toLowerCase()} ${externalBusy?'external-busy':''} ${normalizedQuery&&!externalBusy?'highlighted':''}`} style={{top,height}} title={externalBusy?'Время занято в МИС':undefined}><div className="appt-head"><strong>{externalBusy?'Занято':item.patient_name}</strong><span>{timeLabel(item.starts_at)}</span></div><div className="appt-body"><b>{externalBusy?'Недоступно':item.source||'Запись'}</b><small>{durationMinutes(item)} мин</small></div></article>})}
              </div>)}
            </div>
          </div>
        </div>
