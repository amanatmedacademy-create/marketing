...
import { CalendarDays, ChevronLeft, ChevronRight, Clock3, Phone, Plus, RefreshCw, Search, X } from 'lucide-react';
...
export default function ClinicSchedulePage() {
  ...
  const counters=useMemo(()=>({today:new Set(data.appointments.map((x)=>x.patient_id||x.phone||x.patient_name)).size,arrived:data.appointments.filter((x)=>x.status==='CONFIRMED'||x.status==='COMPLETED').length,completed:data.appointments.filter((x)=>x.status==='COMPLETED').length,cancelled:data.appointments.filter((x)=>x.status==='CANCELLED'||x.status==='NO_SHOW').length}),[data.appointments]);
  ...
  const createDraft=(doctorId:string,minute:number,patient?:Appointment|Patient)=>setDraft({doctorId,patientId:'id' in (patient||{}) ? String((patient as Patient).id||'') : '',patientName:patient ? ('patient_name' in patient ? patient.patient_name : patient.name) : '',phone:patient?.phone||'',date:selectedDate,time:minuteLabel(minute),duration:30,note:''});
  ...
  const planNext=(item:Appointment)=>{ const nextDoctor=item.doctor_id; const nextDate=addDays(selectedDate,1); setSelected(null); setSelectedDate(nextDate); window.setTimeout(()=>setDraft({doctorId:nextDoctor,patientId:item.patient_id||'',patientName:item.patient_name,phone:item.phone||'',date:nextDate,time:'09:00',duration:durationMinutes(item),note:'Следующая запись'}),0); };
