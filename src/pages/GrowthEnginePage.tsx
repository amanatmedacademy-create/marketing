...
import { ArrowRight, RefreshCw, RotateCcw, Target, TrendingUp } from 'lucide-react';
...
export default function GrowthEnginePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Ошибка загрузки Growth Engine');
  ...
  const updateOpportunity = async (id: string, status: LostOpportunity['status']) => {
    try {
      await api(`/api/growth/lost-opportunities/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify({ status }) });
      await load();
  ...
  return <div className="strategic-page">
    ...
    <div className="strategic-grid strategic-grid--equal">
      ...
      <section className="panel">
        <div className="google-card-head"><div><h2>Offline Conversions</h2><p>События нижней части воронки, подготовленные для Meta / Google / TikTok.</p></div><Target size={20}/></div>
        <div className="run-list">{conversions.length ? conversions.map((item) => <div className="run-item" key={item.id}><i className={`run-dot ${item.sync_status === 'sent' ? 'success' : item.sync_status === 'failed' ? 'failed' : ''}`}/><div><b>{labels[item.event_name] || item.event_name} → {item.destination}</b><small>{dateTime(item.occurred_at)}{item.value > 0 ? ` · ${money(item.value, item.currency)}` : ''}</small></div><span className="badge">{item.sync_status}</span></div>) : <div className="suite-state">Конверсионных событий пока нет.</div>}</div>
        {pending.length > 0 && <div className="strategic-note">В очереди {pending.length} событий. Growth Engine не помечает их отправленными, пока конкретный рекламный коннектор фактически не подтвердит доставку.</div>}
      </section>
    </div>
