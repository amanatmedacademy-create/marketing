...
import { fetchFunnelWorkspace, type FunnelLead, type FunnelLeadStage, type FunnelUser } from '../services/salesFunnel';
...
export function LeadsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (filter === 'NO_PHONE' && normalizePhone(lead.phone)) return false;
  ...
  const contactPhone = normalizePhone(selected?.lead.phone);
  ...
  return <div className="leads-workspace">
    ...
    {error && <div className="lead-error"><AlertTriangle size={18}/><span>{error}</span><button onClick={() => setError('')}><X size={16}/></button></div>}
    ...
    {!loading && rows.length > 0 && <main className="lead-layout">
      <section className="lead-table-panel">
        ...
        <div className="lead-table-wrap"><table><thead><tr><th>Клиент</th><th>Источник / канал</th><th>Стадия</th><th>Ответственный</th><th>Следующее действие</th><th>Последний контакт</th></tr></thead><tbody>
          {filtered.map((row) => <tr key={row.lead.id} className={selectedId === row.lead.id ? 'selected' : ''} onClick={() => setSelectedId(row.lead.id)}>
            <td><div className="lead-client"><span>{fullName(row.lead).slice(0, 1).toUpperCase()}</span><div><b>{fullName(row.lead)}</b><small>{row.lead.phone || row.lead.email || row.lead.social_username || 'Контакты не указаны'}</small>{row.duplicateCount > 0 && <em>Возможный дубль · {row.duplicateCount}</em>}</div></div></td>
      ...
      <aside className="lead-card">
        {!selected ? <div className="lead-state">Выберите лида</div> : <>
          <header className="lead-card-head"><div className="lead-avatar">{fullName(selected.lead).slice(0, 2).toUpperCase()}</div><div><span>{selected.lead.external_id || selected.lead.id.slice(0, 8)}</span><h2>{fullName(selected.lead)}</h2><p>{selected.lead.social_username || selected.lead.email || selected.lead.phone || 'Контакты не указаны'}</p></div><button type="button" onClick={openEdit}>Изменить</button></header>
          ...
          <div className="lead-actions">
            ...
            {contactPhone && <a href={`https://wa.me/${contactPhone}`} target="_blank" rel="noreferrer"><ExternalLink size={17}/>WhatsApp</a>}
          ...
          <section><h3>Контакт и источник</h3><dl className="lead-details">
            ...
            <div><dt>Телефон</dt><dd>{selected.lead.phone || '—'}</dd></div><div><dt>Email</dt><dd>{selected.lead.email || '—'}</dd></div>
    ...
    {editOpen && selected && <div className="lead-modal-layer" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setEditOpen(false); }}>
      <form className="lead-modal" onSubmit={(event) => void save(event)}>
        ...
        <div className="lead-modal-body">
          ...
          <label><span>Телефон</span><input value={draft.phone} onChange={(event) => setDraft((value) => ({ ...value, phone: event.target.value }))}/></label>
