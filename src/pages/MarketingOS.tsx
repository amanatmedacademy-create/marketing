...
const date = (value?: string | null) => value ? new Date(`${value}T00:00:00`).toLocaleDateString('ru-RU', { day: '2-digit', month: 'short' }) : '—';
const dateTime = (value?: string | null) => value ? new Date(value).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'Не запускалась';
...
export default function MarketingOS() {
  ...
  const load = async () => {
    ...
    try {
      const [campaignRows, taskRows, contentRows, automationRows, activityRows] = await Promise.all([
        ...
        operationsApi.automations.list(),
        operationsApi.activity.list(),
      ]);
      ...
      setError(reason instanceof Error ? reason.message : 'Не удалось загрузить operations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleAutomation = async (rule: AutomationRule) => {
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'Не удалось обновить автоматизацию');
    }
  };
  ...
  return <div className="stack marketing-os">
    ...
    <section className="panel">
      <div className="panel-title-row"><div><h2>Кампании и инициативы</h2><p className="note">Реальные записи из marketing_campaigns.</p></div><button className="button" onClick={load}><RefreshCw size={16}/> Обновить</button></div>
      {campaigns.length === 0 ? <p className="note">Кампаний пока нет. Добавьте первую запись в Supabase или через API.</p> : <div className="table-wrap"><table><thead><tr><th>Кампания</th><th>Канал</th><th>Цель</th><th>Ответственный</th><th>Бюджет</th><th>Период</th><th>Статус</th></tr></thead><tbody>{campaigns.map(item => <tr key={item.id}><td><b>{item.name}</b></td><td>{item.channel}</td><td>{item.objective}</td><td>{item.owner}</td><td>{money(Number(item.budget || 0))}</td><td>{date(item.starts_on)} — {date(item.ends_on)}</td><td><span className={`badge ${item.status === 'Активна' ? 'badge--green' : item.status === 'План' ? 'badge--amber' : ''}`}>{item.status}</span></td></tr>)}</tbody></table></div>}
    </section>
    ...
    <div className="grid-2">
      <section className="panel">
        ...
        <div className="task-list">{visibleTasks.map(task => <button className={`task-row ${task.done ? 'is-done' : ''}`} key={task.id} onClick={() => void toggleTask(task)}>{task.done ? <CheckCircle2 size={19}/> : <Circle size={19}/>}<span><b>{task.title}</b><small>{task.owner} · до {date(task.due_on)}</small></span><i className={`priority priority--${task.priority.toLowerCase()}`}>{task.priority}</i></button>)}</div>
      </section>
      ...
      <section className="panel">
        ...
        <div className="content-plan">{content.map(item => <article key={item.id}><time>{date(item.publish_on)}</time><div><b>{item.title}</b><small>{item.platform || 'Площадка не указана'} · {item.owner || 'Не назначен'} · {item.production_stage}</small></div><span className={`badge ${item.status === 'Готово' ? 'badge--green' : item.status === 'Сегодня' ? 'badge--amber' : ''}`}>{item.status}</span></article>)}</div>
      </section>
    </div>
